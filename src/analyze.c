...
static void analyzeOneTable(
  ...
  if( pTab->tnum==0 ){   /*tnum表示 表的B树的根节点*/
    /* Do not gather statistics on views or virtual tables */ /*不收集视图或虚拟表的统计信息*/
    return;
  }
  if( memcmp(pTab->zName, "sqlite_", 7)==0 ){  /*判定以 sqlite_ 匹配的表名*/
    /* Do not gather statistics on system tables */  /*不收集系统表的统计信息*/
    return;
  }
  ...
  for(pIdx=pTab->pIndex; pIdx; pIdx=pIdx->pNext){  /*pIndex表示该表的SQL索引列表，pNext表示相关联的同一个表的下一个索引*/
    ...
    /* Populate the register containing the index name. */ /*移植包含该索引名字的寄存器*/
  ...
  */
  ...
  /* 如果表没有索引, 创建一个 包含NULL的 sqlite_stat1 条目
  ** 作为索引名，并且行数作为内容.
  */
  if( pTab->pIndex==0 ){
