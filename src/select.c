...
** 删除所有查询结构的内容但不删除查询结构本身
**仅仅是清除Select对象中所有内容＃即是清除表达式列表中的表达式
*/
static void clearSelect(sqlite3 *db, Select *p){
  sqlite3ExprListDelete(db, p->pEList);   /*清除select结构体中的查询结果*/
  sqlite3SrcListDelete(db, p->pSrc);   /*从表达式列表中清除FROM子句表达式*/
  sqlite3ExprDelete(db, p->pWhere);     /*从表达式列表中清除FROM子句表达式*/
  sqlite3ExprListDelete(db, p->pGroupBy);   /*从表达式列表中清除GroupBy子句表达式*/
  sqlite3ExprDelete(db, p->pHaving);   /*从表达式列表中清除Having子句表达式*/
  sqlite3ExprListDelete(db, p->pOrderBy);   /*从表达式列表中清除OrderBy子句表达式*/
  sqlite3SelectDelete(db, p->pPrior);   /*从表达式列表中清除Prior子句表达式*/
  sqlite3ExprDelete(db, p->pLimit);    /*从表达式列表中清除Limit子句表达式*/
...
**在连接关键字前加一到3个标示符，决定使用何种连接方式。
**返回一个整数常数,表示使用以下的何种连接类型:
**JT_INNER 
**JT_CROSS 
**JT_OUTER 
**JT_NATURAL 
**JT_LEFT 
**JT_RIGHT 
**全外连接是JT_LEFT 和JT_RIGHT 结合。
**如果看到一个非法或不受支持的连接类型,仍然返回一个
**连接类型,但在pParse结构中加入一个错误信息
**
**自己的理解:这个函数的功能是根据连接的关键字处理连接的类型，返回的整数代表连接的类型
**然而在处理的过程中会检查到错误，仍然要返回连接类型，默认的连接类型是inner join。
**
*/
int sqlite3JoinType(Parse *pParse, Token *pA, Token *pB, Token *pC){
  ...
  for(i=0; i<3 && apAll[i]; i++){
    ...
    for(j=0; j<ArraySize(aKeyword); j++){/*循环连接关键字数组*/
      if( p->n==aKeyword[j].nChar /*如果P中字符个数等于连接数组中的关键字长度*/
          && sqlite3StrNICmp((char*)p->z, &zKeyText[aKeyword[j].i], p->n)==0 ){/*使用比较字符串函数比较*/
        jointype |= aKeyword[j].code;/*如果通过了比较长度和内容，返回连接类型*/
        break;
      }
    }
    testcase( j==0 || j==1 || j==2 || j==3 || j==4 || j==5 || j==6 );/*利用测试代码中的testcast,测试j值，是否在这个范围内*/
    if( j>=ArraySize(aKeyword) ){/*如果j比连接关键字数组还大*/
      jointype |= JT_ERROR;/*返回一个错误*/
      break;
    }
  }
  if(
     (jointype & (JT_INNER|JT_OTERU))==(JT_INNER|JT_OUTER) ||/*如果连接类型交上(JT_INNER|JT_OTERU)的结果，与JT_INNER|JT_OUTER一样*/
     (jointype & JT_ERROR)!=0/*或者连接关键字是错误连接*/
  ){
    const char *zSp = " "; /*只读的字符型指针zSp*/
    assert( pB!=0 );/*传入参数中PB不为空*/
    if( pC==0 ){ zSp++; }/*如果指针pC指向的地址为0，那么指针zSp指向的地址前移一个存储单元*/
    sqlite3ErrorMsg(pParse, "unknown or unsupported join type: "  
       "%T %T%s%T", pA, pB, zSp, pC);  /*错误消息unknown or unsupported join type*/
    jointype = JT_INNER;/*默认连接类型是内连接*/
  }else if( (jointype & JT_OUTER)!=0 
         && (jointype & (JT_LEFT|JT_RIGHT))!=JT_LEFT ){/*如果连接类型是JT_OUTER和JT_RIGHT*/
    sqlite3ErrorMsg(pParse, 
      "RIGHT and FULL OUTER JOINs are not currently supported");/*错误消息RIGHT and FULL OUTER JOINs are not currently supported*/
    jointype = JT_INNER;/*此时使用默认的连接类型*/
  }
  return jointype;
}
...
**这个函数是用来添加where子句解释含有JOIN语法句，从而解释select语句，
**这个新语句添加到含有where子句中，格式如下:

**(tab1.col1 = tab2.col2)

**tabl是表集合psrc中第i个表，tab2是表集合第i+1个表，col1是tab1中索引，col2是tab2中索引
**
*/
static void addWhereTerm(
  ...
  assert( iLeft<iRight );/*判断第一个表的索引值是否小于第二个表的索引值*/
  assert( pSrc->nSrc>iRight );/*判断表集合中的表的数目是否大于右表的索引值*/
  assert( pSrc->a[iLeft].pTab );/*判断表集合中左表索引的表是否为空*/
  assert( pSrc->a[iRight].pTab );/*判断表集合中右表索引的表是否为空*/
  ...
  pEq = sqlite3PExpr(pParse, TK_EQ, pE1, pE2, 0);/*分配一个额外的节点连接这两个子树表达式*/
  if( pEq && isOuterJoin ){/*如果pEq表达式是外连接表达式*/
    ExprSetProperty(pEq, EP_FromJoin);/*在连接中使用ON或者USING子句*/
    assert( !ExprHasAnyProperty(pEq, EP_TokenOnly|EP_Reduced) );/*判断pEq表达式是否是EP_TokenOnly|EP_Reduced*/
    ExprSetIrreducible(pEq);/*调试pEq，设置是否可以约束*/
    pEq->iRightJoinTable = (i16)pE2->iTable;/*指定要连接的右表是第二个表达式的表*/
  }
  *ppWhere = sqlite3ExprAnd(db, *ppWhere, pEq);/*对指定数据库的表达式进行连接*/
}
...
**在给出的所有的表达式中，设定 EP_FromJoin属性，并且在表达式中设定 Expr.iRightJoinTable ，
** EP_FromJoin属性作为表达式的条款表达式左外连接的处理逻辑，它是ON或者USING特定限制连接中的一部分，
**但通常不作为WHERE子句的一部分，这些术语在表连接处理中移植到where中使用，它来源于on和using子句中
** Expr.iRightJoinTable 告诉where子句处理要依靠iRightJoinTable表，即使这个表不能明确的表达，所需要的信息如下澹?
**SELECT * FROM t1 LEFT JOIN t2 ON t1.a=t2.b AND t1.x=5
**where子句需要延时处理t1.x=5,知道t2循环连接完毕，通过这种方式，t2中一个空值，即使t1.x!=5也会被选择，如果没有
**没有定义处理t1.x=5，将会在t1循环完后再处理列中t1.x!=5不符合要求就不会输出。
**自己的理解:该函数进行处理FROM后表的连接方式，根据返回连接类型，设置连接表达式。
**
**
*/
...
static int multiSelect(
  ...
  explainComposite(pParse, p->op, iSub1, iSub2, p->op!=TK_ALL);/*如果没有执行SELECT，那么就输出Composite subqueries  iSub1和 iSub2*/
	...
	*/
	if( p->selFlags & SF_UsesEphemeral ){/*如果SELECT中selFlags为SF_UsesEphemeral*/
	  ...
	  nCol = p->pEList->nExpr;/*将表达式列表中表达式的个数赋值给	  nCol*/
	  ...
	  if( !pKeyInfo ){/*如果关键信息结构体不存在*/
		...
		goto multi_select_end;/*跳到multi_select_end*/
	  }
  
	  pKeyInfo->enc = ENC(db);/*设置关键信息结构体的环境变量为db*/
	  pKeyInfo->nField = (u16)nCol;/*设置关键结构体的域的大小为结果集的列数*/
  
	  for(i=0, apColl=pKeyInfo->aColl; i<nCol; i++, apColl++){/*结果集的列数循环*/
	  ...
	  for(pLoop=p; pLoop; pLoop=pLoop->pPrior){/*遍历优先SELECT*/
		for(i=0; i<2; i++){
		  int addr = pLoop->addrOpenEphm[i];/*将OP_OpenEphem操作数组的元素赋值给 addr地址*/
		  if( addr<0 ){/*如果addr小于0*/
			/* If [0] is unused then [1] is also unused.  So we can
			** always safely abort as soon as the first unused slot is found 
			**如果[0]没有被使用，所以[1]也没有被使用. 所以我们总能尽快安全地终止第一个发现未使用的位置 
			*/
			assert( pLoop->addrOpenEphm[1]<0 );/*插入断点，判断循环中OP_OpenEphem操作数组小于0的情况*/
			break;
		  }
		  sqlite3VdbeChangeP2(v, addr, nCol);/*将ncol的值设置到虚拟机*/
		  sqlite3VdbeChangeP4(v, addr, (char*)pKeyInfo, P4_KEYINFO);/*如果关键信息结构中地址的值大于addr，将P4_KEYINFO的值设置到虚拟机*/
		  pLoop->addrOpenEphm[i] = -1;/*将OP_OpenEphem操作数组的元素设置为-1*/
		}
	  }
	  sqlite3DbFree(db, pKeyInfo);/*释放数据库连接中关联关键字结构体的内存*/
	}
  
  multi_select_end:/*上文中的goto跳转到这部分*/
	pDest->iSdst = dest.iSdst;/*将基址寄存器的的值赋值给处理结果集的存储基址寄存器的值*/
	pDest->nSdst = dest.nSdst;/*已分配基址寄存器的数量赋值给处理结果集的存储基址寄存器的数量*/
	sqlite3SelectDelete(db, pDelete);/*释放SELECT结构体pselect*/
	return rc;
}
  #endif /* SQLITE_OMIT_COMPOUND_SELECT */
  ...
  ** iBreak.
  **为协作程序写一个输出子程序实现SELECT语句，输出的数据来自pin-iSdst(存放结果的基址寄存器)，
  **pin-iSdst的列被输出，pDest(处理结果集结构体)进行输出，regReturn是存储子程序地址的寄存器的数量，
  **如果regRrev>0,那么它是第一个向量寄存器，其中记录着前面的输出数据，
  **如果在SELECT结构体中找到limit，立即跳到iBreak.
 */
 static int generateOutputSubroutine(
	Parse *pParse,			/* Parsing context *//*解析上下文*/
	Select *p,				/* The SELECT statement *//*声明SELECT结构体*/
	SelectDest *pIn,		/* Coroutine supplying data *//*提供数据*/
	SelectDest *pDest,		/* Where to send the data *//*发送数据*/
	int regReturn,			/* The return address register *//*返回地址寄存器*/
	int regPrev,			/* Previous result register.  No uniqueness if 0 *//*以前的结果寄存器，如果为0就不唯一*/
	KeyInfo *pKeyInfo,		/* For comparing with previous entry *//*与以前条目的比较*/
	int p4type, 			/* The p4 type for pKeyInfo *//* pKeyInfo的4种类型*/
	int iBreak				/* Jump here if we hit the LIMIT *//*如果发现LIMIT就跳到此处*/
  ){
	Vdbe *v = pParse->pVdbe;/*将语法解析树的VDBE属性赋值给VDBE变量*/
	int iContinue;
	int addr;
  
	addr = sqlite3VdbeCurrentAddr(v);/*将VDBE当前的指令地址赋值给addr*/
	iContinue = sqlite3VdbeMakeLabel(v);/*为VDBE创建一个标签，返回值赋给iContinue*/
  
	/* Suppress duplicates for UNION, EXCEPT, and INTERSECT 
	*/
	if( regPrev ){/*如果以前的寄存器存在*/
	  int j1, j2;
	  j1 = sqlite3VdbeAddOp1(v, OP_IfNot, regPrev);/*将 OP_IfNot操作交给VDBE，然后返回这个操作的地址*/
	  j2 = sqlite3VdbeAddOp4(v, OP_Compare, pIn->iSdst, regPrev+1, pIn->nSdst,
								(char*)pKeyInfo, p4type);/*在操作中添加 OP_IfNot，并返回一个指针*/
	  sqlite3VdbeAddOp3(v, OP_Jump, j2+2, iContinue, j2+2);/*将 OP_Jump操作交给VDBE，然后返回这个操作的地址*/
	  sqlite3VdbeJumpHere(v, j1);/*如果j1存在，运行地址跳转到j1*/
	  sqlite3ExprCodeCopy(pParse, pIn->iSdst, regPrev+1, pIn->nSdst);/*复制参数中pIn->iSdst的内容到pIn->nSdst*/
	  sqlite3VdbeAddOp2(v, OP_Integer, 1, regPrev);/*将 OP_Integer操作交给VDBE，然后返回这个操作的地址*/
	}
	if( pParse->db->mallocFailed ) return 0;/*如果内存分配失败，就直接返回0*/
  
	/* Suppress the first OFFSET entries if there is an OFFSET clause/*如果有OFFSET 子句，阻止第一个OFFSET 进入*/
	*/
	...
	switch( pDest->eDest ){
	  ...
	  */
	  case SRT_Table:
	  case SRT_EphemTab: {
		int r1 = sqlite3GetTempReg(pParse);/*把语法解析树中临时寄存器的值给r1*/
		int r2 = sqlite3GetTempReg(pParse);/*把语法解析树中临时寄存器的值给r2*/
		...
		sqlite3VdbeAddOp2(v,OP_NewRowid, pDest->iSDParm, r2);/*将 OP_NewRowid操作交给VDBE，然后返回这个操作的地址*/
		sqlite3VdbeAddOp3(v, OP_Insert, pDest->iSDParm, r1, r2);/*将 OP_Insert操作交给VDBE，然后返回这个操作的地址*/
		sqlite3VdbeChangeP5(v, OPFLAG_APPEND);
	  ...
	  ** then there should be a single item on the stack.  Write this
	  ** item into the set table with bogus data.
	  **如果为expr IN (SELECT ...)结构创建了一个集合，然后应该有一个单独的条目在堆中，
	  **使用虚拟数据写这个条目到这个集合表中
	  */
	  case SRT_Set: {
		int r1;
		...
		p->affinity = 
		   sqlite3CompareAffinity(p->pEList->a[0].pExpr, pDest->affSdst);/*将表达式列表中的表达式pExpr与处理结果集中的affSdst比较，并返回比较后的类型*/
		r1 = sqlite3GetTempReg(pParse);/*把语法解析树中临时寄存器中的值给r1*/
		sqlite3VdbeAddOp4(v, OP_MakeRecord, pIn->iSdst, 1, r1, &p->affinity, 1);/*将 OP_MakeRecord操作交给VDBE，然后返回这个操作的地址*/
		sqlite3ExprCacheAffinityChange(pParse, pIn->iSdst, 1);
		sqlite3VdbeAddOp2(v, OP_IdxInsert, pDest->iSDParm, r1);/*将 OP_IdxInsert操作交给VDBE，然后返回这个操作的地址*/
		sqlite3ReleaseTempReg(pParse, r1);/*释放语法解析树中临时寄存器r1的值*/
		break;
	  }
...
*/
case SRT_Exists: {
		sqlite3VdbeAddOp2(v, OP_Integer, 1, pDest->iSDParm);/*将 OP_Integer操作交给VDBE，然后返回这个操作的地址*/
		/* The LIMIT clause will terminate the loop for us *//*LINIT子句将会终止循环*/
		break;
	  }
#endif
	  ...
	  ** store the results in the appropriate memory cell and break out
	  ** of the scan loop.
	  **如果一个选择是一个表达式的一部分，将这个结果存储在一个合适的内存中，这个扫描循环就结束
	  */
	  case SRT_Mem: {
		assert( pIn->nSdst==1 );/*插入断点，判断协同处理数据集的寄存器是否为1*/
		sqlite3ExprCodeMove(pParse, pIn->iSdst, pDest->iSDParm, 1);/*释放寄存器的内容，保持寄存器的内容及时更新*/
		/* The LIMIT clause will jump out of the loop for us *//*Limit子句将会跳出这个循环*/
		break;
	  }
#endif /* #ifndef SQLITE_OMIT_SUBQUERY */
	  ...
	  **结果存储在一个连续的寄存器中，起始位置为写入数据的基址寄存器的的地址，然后执行协同代码块*/
	  */
	  case SRT_Coroutine: {
		if( pDest->iSdst==0 ){/*如果基址寄存器为0*/
		  pDest->iSdst = sqlite3GetTempRange(pParse, pIn->nSdst);/*分配一串连续的寄存器，长度为已分配寄存器的个数*/
		  pDest->nSdst = pIn->nSdst;/*将分配寄存器的个数记录到nSdst中*/
		}
		sqlite3ExprCodeMove(pParse, pIn->iSdst, pDest->iSdst, pDest->nSdst);/*释放寄存器的内容，保持寄存器的内容及时更新*/
		sqlite3VdbeAddOp1(v, OP_Yield, pDest->iSDParm);/*将 OP_IdxInsert操作交给VDBE，然后返回这个操作的地址*/
		break;
	  }
	  ...
	  **对于结果输出来说，结果存储在一个寄存器序列中，然后OP_ResultRow操作用在sqlite3_step() 中，返回下一行的结果
	  */
	  default: {
		assert( pDest->eDest==SRT_Output );/*插入断点，判断处理结果集中处理方式是否为sqlite3_step() */
		sqlite3VdbeAddOp2(v, OP_ResultRow, pIn->iSdst, pIn->nSdst);/*将 OP_ResultRow操作交给VDBE，然后返回这个操作的地址*/
	...
	*/
	if( p->iLimit ){
	  sqlite3VdbeAddOp3(v, OP_IfZero, p->iLimit, iBreak, -1);/*将 OP_IfZero操作交给VDBE，然后返回这个操作的地址*/
	}
	...
	*/
	...
	sqlite3VdbeAddOp1(v, OP_Return, regReturn);/*将 OP_Return操作交给VDBE，然后返回这个操作的地址*/
  ...
  **我们假设一个如下的查询方式:
  **<selectA>  <operator>  <selectB>	ORDER BY <orderbylist>
  **操作符是 UNION ALL, UNION, EXCEPT, or INTERSECT之一，是将均含ORDERBY的<selectA>和<selectB>作为协同程序。
  **并行运行协同程序，合成结果再输出，除了这2个协同程序，下面还有7个子程序:
  **OUTA:将SELECTA的输出结果移动到复合查询结果中
  **OUTB:将SELECTB的输出结果移动到复合查询结果中,(只发生在UNION ALL, UNION, EXCEPT, or INTERSECT，不会输出只有B中才有的行)
  **ALTB:当数据来自这个协同程序，并且A<B时被调用
  **AeqB:当数据来自这个协同程序，并且A=B时被调用
  **AgtB:当数据来自这个协同程序，并且A>B时被调用
  **EofA当用尽A中的数据(实际上是循环A表，循环完毕)
  **EofB当用尽A中的数据(实际上是循环B表，循环完毕)
  ...
  **
  **   EofB:   outA, nextA		outA, nextA 	  outA, nextA		  halt
  **在AltB,AeqB,和AgtB子程序中，一个EOF出现在A中，然后nextA引起直接跳到EofA.
  **EOF出现在b中，然后nextb引起直接跳到EofB.在EofA和EofB里面，一个EOF出现在条目上或者接下来一个next一个挑战到
  **SELECT处理的结尾。
  **<<去除重复的部分>>，在输出子程序中处理，删除在 UNION, EXCEPT和INTERSECT的副本。
  **在regPrev寄存器中保存已输出的数据，对这个存储的值和输出的值进行比较，比较是否相同。
  **
  **<<这个实现的方法是先实现两个协同程序和7个子程序，然后在结尾加入使用控制逻辑>>，如下:
  **		  goto Init
  **	 coA: coroutine for left query (A)
  **	 coB: coroutine for right query (B)
  **	outA: output one row of A
  ...
  **		  Jump AltB, AeqB, AgtB
  **	 End: ...
  **我们能调用AltA,AeqB, AgtB,EofA和EofB的子程序，但是实际上他们没有返回的时候，我们不能调用她们。
  **直到所有的数据都已经使用过了，EofA和EofB的循环才跳到"end"标签处。AltA,AeqB, AgtB跳到L2或者跳到EofA和EofB	其中之一。
  */
#ifndef SQLITE_OMIT_COMPOUND_SELECT
  static int multiSelectOrderBy(
	Parse *pParse,		  /* Parsing context *//*解析上下文*/
	...
	int i, j;			  /* Loop counters *//*循环计算器*/
	Select *pPrior; 	  /* Another SELECT immediately to our left *//*在左边的立即执行*/
	Vdbe *v;			  /* Generate code to this VDBE *//*在虚拟机中生成代码*/
	...
	int addrOutA;		  /* Address of the output-A subroutine *//* output-A程序的地址*/
	int addrOutB = 0;	  /* Address of the output-B subroutine *//* output-B程序的地址*/
	int addrEofA;		  /* Address of the select-A-exhausted subroutine *//*select-A-exhausted 程序的地址*/
	int addrEofB;		  /* Address of the select-B-exhausted subroutine *//*select-B-exhausted 程序的地址*/
	...
	int regLimitA;		  /* Limit register for select-A *//*select-A的limit寄存器*/
	int regLimitB;		  /* Limit register for select-A *//*select-B的limit寄存器*/
	int regPrev;		  /* A range of registers to hold previous output *//*一系列保存之前的输出的寄存器*/
	int savedLimit; 	  /* Saved value of p->iLimit *//*保存 p->iLimit的值*/
	int savedOffset;	  /* Saved value of p->iOffset *//*保存  p->iOffset的值*/
	...
	int j1; 			  /* Jump instructions that get retargetted *//*获得新指令的跳转指令*/
	...
	int nOrderBy;		  /* Number of terms in the ORDER BY clause *//*ORDER BY子句的个数*/
	int *aPermute;		  /* Mapping from ORDER BY terms to result set columns *//*做从ORDER BY到结果集的映射*/
#ifndef SQLITE_OMIT_EXPLAIN
	...
	labelEnd = sqlite3VdbeMakeLabel(v);/*为VDBE创建一个标签，返回值赋给labelEnd*/
	labelCmpr = sqlite3VdbeMakeLabel(v);/*为VDBE创建一个标签，返回值赋给	labelCmp*/
  
  
	/* Patch up the ORDER BY clause
	*/
	op = p->op;  /*将SELECT中的操作符赋值给op*/
	pPrior = p->pPrior;/*SELECT结构体的优先查询赋值给?淞縫Prior*/
	assert( pPrior->pOrderBy==0 );/*插入断点，如果优先查询的SELECT没有OrderB，便抛出警告错误*/
	pOrderBy = p->pOrderBy;/*将SELECT中的pOrderBy子句赋值给pOrderBy;变量*/
	assert( pOrderBy );/*插入断点，如果优先查询的SELECT没有OrderB，便抛出警告错误*/
	nOrderBy = pOrderBy->nExpr;/*pOrderBy表达式个数赋值给nOrderBy*/
  
	/* For operators other than UNION ALL we have to make sure that
	** the ORDER BY clause covers every term of the result set.  Add
	** terms to the ORDER BY clause as necessary.
	**除了UNION ALL， ORDER BY必须要覆盖到结果集的每一个条款*/
	*/
	if( op!=TK_ALL ){/*如果操作符不是TK_ALL*/
	  for(i=1; db->mallocFailed==0 && i<=p->pEList->nExpr; i++){/*遍历表达式*/
		struct ExprList_item *pItem;/*声明一个表示列表项*/
		for(j=0, pItem=pOrderBy->a; j<nOrderBy; j++, pItem++){/*遍历条目*/
		  assert( pItem->iOrderByCol>0 );/*插入断点，如果没有OrderByCol，就抛出错误信息*/
		  if( pItem->iOrderByCol==i ) break;/*如果OrderBy条目为i，便返回，跳出当前的循环*/
		}
		if( j==nOrderBy ){/*如果此时上个循环结束，j的值为OrderBy的个数*/
		  ...
		  pNew->flags |= EP_IntValue;/*flgs标签位或EP_IntValue再赋值给flags*/
		  pNew->u.iValue = i;/*将当前i的值，赋值给表达式中i的值*/
	...
	** collation.
	**用排列计算比较排列和关键信息结构决定下一行结果来自selectA or selectB，
	**添加对ORDER BY子句显示排列，以便评估右边和左边的子程序，让其使用正确的序列。
	*/
	...
	if( aPermute ){/*如果aPermute不为0*/
	  ...
	  for(i=0, pItem=pOrderBy->a; i<nOrderBy; i++, pItem++){/*遍历表达式列表项*/
		assert( pItem->iOrderByCol>0  && pItem->iOrderByCol<=p->pEList->nExpr );/*插入断点，判断根据列OrderBy个数大于0并且小于等于表达式个数*/
		aPermute[i] = pItem->iOrderByCol - 1;/*将根据列OrderBy减1存到aPermute数组中*/
	  }
	  pKeyMerge =
		sqlite3DbMallocRaw(db, sizeof(*pKeyMerge)+nOrderBy*(sizeof(CollSeq*)+1));/*分配并清空内存，再分配大小为第二个参数*/
	  if( pKeyMerge ){/*如果上句程序中pKeyMerge不为空*/
		pKeyMerge->aSortOrder = (u8*)&pKeyMerge->aColl[nOrderBy];/*关键信息结构体中存储队列排序的方法，值为关键信息机构体中aColl数组存储的方式*/
		pKeyMerge->nField = (u16)nOrderBy;/*OrderBy的个数存入为关键信息结构体的aColl[]的条目个数*/
		pKeyMerge->enc = ENC(db);/*设定编码方式*/
		for(i=0; i<nOrderBy; i++){/*根据OrderBy的个数遍历*/
		  ...
		  if( pTerm->flags & EP_ExpCollate ){/*如果flags的值为EP_ExpCollate*/
			pColl = pTerm->pColl;/*将表达式pTerm排序序列赋值给变量pColl*/
		  }else{
			pColl = multiSelectCollSeq(pParse, p, aPermute[i]);/*为列生成一个排序序列*/
			pTerm->flags |= EP_ExpCollate;
			pTerm->pColl = pColl;/*将已经存入数据的pColl赋值给表达式中的排序序列*/
		  }
		  pKeyMerge->aColl[i] = pColl;/*将当前循环中排列序列放到关键信息结构中序列组中，并对应在i下*/
		  pKeyMerge->aSortOrder[i] = pOrderBy->a[i].sortOrder;/*将pOrderBy表达式列表中的第i-1个排序方法放到关键信息结构体中的排序数组中*/
		}
	  }
	}else{/*如果上句程序中 pKeyMerge不为空，说明表达式为空，内存中没有*/
	  pKeyMerge = 0;/*将关键信息结构体置为0*/
	}
  
	/* Reattach the ORDER BY clause to the query./*再次将ORDER BY子句放到查询中*/
	*/
	p->pOrderBy = pOrderBy;/*将当前的pOrderBy表达式列表赋值给SELECT的OrderBy属性*/
	pPrior->pOrderBy = sqlite3ExprListDup(pParse->db, pOrderBy, 0);/*深度赋值属性pOrderBy给优先查找SELECT的pOrderBy*/
  
	/* Allocate a range of temporary registers and the KeyInfo needed
	** for the logic that removes duplicate result rows when the
	** operator is UNION, EXCEPT, or INTERSECT (but not UNION ALL).
	**当操作符是UNION, EXCEPT, or INTERSECT(除了 UNION ALL)时，分配一套临时寄存器和逻辑上需要的关键信息结构体，删除重复飞结果*/
	*/
	if( op==TK_ALL ){/*当操作符是TK_ALL*/
	  regPrev = 0;/*将存储以前的数据的寄存器置0*/
	}else{
	  int nExpr = p->pEList->nExpr;/*将表达式列表中表达式的个数赋值给nExpr*/
	  assert( nOrderBy>=nExpr || db->mallocFailed );/*插入断点，如果OrderBy的个数大于等于nExpr或者内存分配失败*/
	  ...
	  pKeyDup = sqlite3DbMallocZero(db,
					sizeof(*pKeyDup) + nExpr*(sizeof(CollSeq*)+1) );/*分配并清空内存，分配大小为第二个参数*/
	  if( pKeyDup ){/*如果上一句分配内存的大小不为0*/
		pKeyDup->aSortOrder = (u8*)&pKeyDup->aColl[nExpr];/*将排序序列表达式赋值给根据信息结构中的排序属性*/
		pKeyDup->nField = (u16)nExpr;/*将表达式个数赋值给aCOLL数组中的总的条目*/
		pKeyDup->enc = ENC(db);/*设定编码方式*/
		for(i=0; i<nExpr; i++){/*遍历表达式*/
		  pKeyDup->aColl[i] = multiSelectCollSeq(pParse, p, i);/* multiSelectCollSeq设置排序的方法，再赋值给aCOLL*/
		  pKeyDup->aSortOrder[i] = 0;/*设置完排序序列后，将排序方法设为0*/
		}
	  }
	}
	...
	**单独的左边和右边查询
	*/
	p->pPrior = 0;/*令SELECT中的优先查询为空*/
	sqlite3ResolveOrderGroupBy(pParse, p, p->pOrderBy, "ORDER");/*解析GroupBy中OrderBy，检测OrderBy关键字，返回0表示成功*/
	if( pPrior->pPrior==0 ){/*如果SELECT中的优先查询为空*/
	  sqlite3ResolveOrderGroupBy(pParse, pPrior, pPrior->pOrderBy, "ORDER");/*解析GroupBy中OrderBy，检测OrderBy关键字，返回0表示成功*/
	}
  
	/* Compute the limit registers */
	/*计算LIMIT寄存器*/
	computeLimitRegisters(pParse, p, labelEnd);/*根据返回的专用地址，计算出LIMIT结果域，并且结束*/
	if( p->iLimit && op==TK_ALL ){/*如果查询中的LIMIT不为空，并且操作符为TK_ALL */
	  regLimitA = ++pParse->nMem;/*将语法解析树中分配内存的数++后赋值给SELECT-A的LIMIT寄存器*/
	  regLimitB = ++pParse->nMem;/*将语法解析树中分配内存的数++后赋值给SELECT-B的LIMIT寄存器*/
	  sqlite3VdbeAddOp2(v, OP_Copy, p->iOffset ? p->iOffset+1 : p->iLimit,
									regLimitA);/*将 OP_Copy操作交给VDBE，然后返回这个操作的地址，如果Offset为空，将offset加1，不为空设置参数为LIMIT的值*/
	  sqlite3VdbeAddOp2(v, OP_Copy, regLimitA, regLimitB);/*将OP_Copy操作交给VDBE，然后返回这个操作的地址*/
	}else{
	  regLimitA = regLimitB = 0;/*否则设置regLimitA 和regLimitB的LIMIT寄存器值为0*/
	}
	sqlite3ExprDelete(db, p->pLimit);/*删除数据库连接的LIMIT*/
	p->pLimit = 0;/*令SELECT结构体中的LIMIT为0*/
	sqlite3ExprDelete(db, p->pOffset);/*删除数据库连接的Offset*/
	p->pOffset = 0;/*令SELECT结构体中Offset 为0*/
  
	regAddrA = ++pParse->nMem;/*select-A程序的基址寄存器为语法解析树中分配内存的数目加1*/
	regEofA = ++pParse->nMem;/*select-A程序的完成标记为语法解析树中分配内存的数目加1*/
	regAddrB = ++pParse->nMem;/*select-B查询的基址寄存器为语法解析树中分配内存的数目加1*/
	regEofB = ++pParse->nMem;/*select-B程序的完成标记为语法解析树中分配内存的数目加1*/
	regOutA = ++pParse->nMem;/*out-A查询的基址寄存器为语法解析树中分配内存的数目加1*/
	regOutB = ++pParse->nMem;/*out-b查询的基址寄存器为语法解析树中分配内存的数目加1*/
	sqlite3SelectDestInit(&destA, SRT_Coroutine, regAddrA);/*初始化处理结果集存储到 regAddrA，标记为SRT_Coroutine*/
	sqlite3SelectDestInit(&destB, SRT_Coroutine, regAddrB);/*初始化处理结果集存储到 regAddrB，标记为SRT_Coroutine*/
  
	/* Jump past the various subroutines and coroutines to the main
	** merge loop
	**跳过各个子程序和协同程序，进入主函数合并循环
	*/
	j1 = sqlite3VdbeAddOp0(v, OP_Goto);/*这个是使用GOTO语句后，返回的地址*/
	addrSelectA = sqlite3VdbeCurrentAddr(v);/*将VDBE的下一条指令地址赋值给addrSelectA的地址*/
  
  
	/* Generate a coroutine to evaluate the SELECT statement to the
	** left of the compound operator - the "A" select.
	**生成一个协同程序，、评估SELECT的左边复杂操作符*/
	*/
	VdbeNoopComment((v, "Begin coroutine for left SELECT"));/*输出提示信息*/
	   ...
	   */  
	   static int flattenSubquery(
		 ...
		 *//*删除关联子查询的临时表结构*/
		 ...
		 sqlite3DbFree(db, pSubitem->zAlias);/*释放数据库连接中存储子查询依赖关系（关联的其他查询）的内存(alias指别名)*/
	   ...
	   *//*（自己注：如果语法语义正确，那就解析，如果没有包含关键字，就默认，如果有max或min就做相应的输出）*/
	   ...
	   */
	   /* （自己注：把count当做第一个参数，说明是一个聚集函数。把tab当做第二个参数，说明是存储数据的地方）
	   ** （代码讲解的真清楚，mark,学习！）
	   ...
	   */
	   /* （自己注：查找一个索引，相当于查找的查找，要根据在表达式列表中索引名查找）
			 ...
			 if( pE->op!=TK_ALL && (pE->op!=TK_DOT || pE->pRight->op!=TK_ALL) ){/*如果表达式的操作不为TK_ALL，并且表达式的操作不为TK_DOT或不为TK_ALL*/
			   ...
			   ** expanded. *//*这"*" 或 "TABLE.*"被需要扩展的表达式*/
			   ...
			   for(i=0, pFrom=pTabList->a; i<pTabList->nSrc; i++, pFrom++){/*遍历表达式列表*/
				 ...
				 char *zTabName = pFrom->zAlias;/*将FROM子句列表项中依赖(alias指别名)赋值给zTabName*/
   ...
   # define explainSimpleCount(a,b,c)
	   ...
	   */
	   /* 自己注：该函数对SQL语句进行语义分析，然后再进行优化，最后生成执行计划。 
	   ** 
	   ** 为给出p参数编写SELECT语句。 
		 ...
		 *//*使用不重复集合打开一个虚拟索引*/
		 ...
		 if( !isAgg && pGroupBy==0 ){/*如果isAgg没有GroupBy*/
		   ...
		   *//*数据库扫描循环结束。*/
		   ...
		   /*自己注：sqlite3WhereEnd主要完成嵌套循环的收尾工作的opcode生成，为每层循环生成OP_Next/OP_Prev，以及关闭表和索引游标的OP_Close*/
		 }else{
		   ...
		   *//*处理带有GROUP BY的聚集函数不同于不带的，而且更为复杂*/
		   if( pGroupBy ){/*(自己注：KeyInfo用来存储与语句相关的关键信息)*/
			 ...
			 *//*（自己注：若实现分组，先进行排序）如果有一个GROUP BY子句，我们可能需要一个排序索引去实现它。
			 ...
			 *//*启动一个循环，提取GROUP BY命令的所有的原列。（自己注：循环提取GROUP BY子句指定的列）*/
			 ...
			 *//*编一个子程序，用来重置group-by累加器*/
   ...
   #endif /* SQLITE_OMIT_BTREECOUNT */
			 {
			   ...
			   *//*处理聚集函数中没有 GROUP BY情况。这个处理程序很简单，只有一个单独的行输出。*/
			   ...
			   sqlite3WhereEnd(pWInfo);/*根据sqlite3WhereBegin()返回信息，生成VDBE的操作码，即执行计划。*/
			   finalizeAggFunctions(pParse, &sAggInfo);/*为AggInfo结构体sAggInfo中每一个聚集函数调用OP_AggFinalize（完成）操作*/
			 }
	   ...
	   static void explainOneSelect(Vdbe *pVdbe, Select *p){
		 ...
		 if( p->pSrc && p->pSrc->nSrc ){/*遍历FROM子句表达式列表*/
		   ...
		   for(i=0; i<p->pSrc->nSrc; i++){/*遍历FROM子句表达式列表**/
			 ...
			 if( pItem->zAlias ){/*如果表达式列表中有关联依赖(alias指别名)*/
