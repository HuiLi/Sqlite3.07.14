...
**/*WAL头长度为32个字节，包括以下8个32位的大端无符号整数。Magic number可以是0x377f0682 或者 0x377f0683；
**数据库页的大小与对应的数据库相同；Checkpoint序列表示经过的Checkpoint次数；
**Salt-1和Salt-2都是随机数，随着Checkpoint的执行前者增加1，后者随机变化；checksum -1和Checksum-2分别是文件头校验和的第1和第2部分 */
...
*/ 
/*
**最大的版本或者唯一的版本的预写日志和预写日志的格式，可通过可通过此版本的SQLite的解释。
**如果客户机开始回收WAL文件，发现（a）该校验码值是正确的和(b）该版本字段不是正确的。
**其产生的结果为WAL_MAX_VERSION恢复失败和SQLite返回SQLITE_CANTOPEN。
*/
...
/*
** The following object holds a copy of the wal-index header content.    以下内容包含 Wal索引 头的内容
**
** The actual header in the wal-index consists of two copies of this  
** object. 实际上wal索引头包括两个部分
**
...
struct WalIndexHdr {
  ...
  u32 unused;                 /* Unused (padding) field */     不用的地方     
  u32 iChange;                /* Counter incremented each transaction */记录每个事务的增长
  u8 isInit;                  /* 1 when initialized */ 当初始化时是  1
  u8 bigEndCksum;             /* True if checksums in WAL are big-endian */如果在WAl的总和检查是二进制的话则为true
  ...
  u32 aCksum[2];              /* Checksum over all prior fields */所有字段进行校和
};
...
*/
struct WalCkptInfo {                      
  u32 nBackfill;                  /* Number of WAL frames backfilled into DB */  有多少个Wal 框回填到DB
  u32 aReadMark[WAL_NREADER];     /* Reader marks */定义读的标志
};
...
*/
/*WALINDEX_LOCK_RESERVED的字节块起始于被锁保留的WALINDEX_LOCK_OFFSET
** 。由于某些系统只支持强制文件锁，我们不会读取或写入数据在其上的被锁的文件的区域.
*/
...
/* Size of write ahead log header, including checksum. */之前写日志头的大小,包括校验和。
...
** all data as 32-bit little-endian words.WAL魔法值。这个值,或至少相同的值有效位设置(WAL_MAGIC | 0 x00000001)存储在32位大端格式WAL的前4个字节的文件。
**如果设置LSB,然后在其内的每一帧的校验和文件处理所有的数据计算了一个32位的数组大端法的话。否则,计算它们的解释所有数据作为32位低位优先的单词。
*/
...
*/
/*
**返回作为弥补的框架，内嵌的框架在先前所写的日志文件中。
**假设记录数据库页码大小的字节是启动预写日志的框架头。
*/
...
struct Wal {
  ...
  u32 iCallback;             /* Value to pass to log callback (or 0) */传递给回收日志值的大小
  i64 mxWalSize;             /* Truncate WAL to this size upon reset */根据重新设定的预写日志的大小进行截取。
  int nWiData;               /* Size of array apWiData */声明数组apWiData的大小
  int szFirstBlock;          /* Size of first block written to WAL file */定义预写日志文件中第一批写的块的大小
  volatile u32 **apWiData;   /* Pointer to wal-index content in memory */**指针大小
  u32 szPage;                /* Database page size */定义数据库页大小
  i16 readLock;              /* Which read lock is being held.  -1 for none */那种读锁被持有。-1 表示没有
  u8 syncFlags;              /* Flags to use to sync header writes */标记是用来同步也的写操作
  u8 exclusiveMode;          /* Non-zero if connection is in exclusive mode */
  u8 writeLock;              /* True if in a write transaction */                如果在做一个写事务时则值为真
  ...
  const char *zWalName;      /* Name of WAL file */定义预写日志文件的名字
  u32 nCkpt;                 /* Checkpoint sequence counter in the wal-header */wal-header检查点序列计数器
#ifdef SQLITE_DEBUG
  u8 lockError;              /* True if a locking error has occurred */当锁定一个错误是则值为真
#endif
};
...
*/
#define WAL_RDWR        0    /* Normal read/write connection */正常的读写连接值为0
#define WAL_RDONLY      1    /* The WAL file is readonly */该预写日志文件是只读的值为1
#define WAL_SHM_RDONLY  2    /* The SHM file is readonly */该共享存储文件是只读的值为2
...
** the largest index).这个结构是用来实现迭代器遍历WAL在数据库中的所有帧页面顺序。两个或两个以上的帧在哪里对应于相同的数据库页面,迭代器只访问帧最近写入的WAL( ** 换句话说,帧是最大的索引)
**
** The internals of this structure are only accessed by: 这种结构的内部只能被以下方式访问。
**
**   walIteratorInit() - Create a new iterator, 创建迭代
**   walIteratorNext() - Step an iterator,        然后 进行下一步
**   walIteratorFree() - Free an iterator.   最后释放迭代
**
** This functionality is used by the checkpoint code (see walCheckpoint()). 这个函数是用于checkpoint 
*/
struct WalIterator {
  int iPrior;                     /* Last result returned from the iterator */ 迭代器中最后的返回值
  int nSegment;                   /* Number of entries in aSegment[] */  aSegment中项目数
...
*/
/*
**定义wal-index 文件中哈希表的参数
**wal-index文件中在每一个HASHTABLE_NPAGE 页码后面都有一个哈希表
**改变任何一个常量将会改变wal-index的格式造成不一致。
*/
#define HASHTABLE_NPAGE      4096                 /* Must be power of 2 */HASHTABLE_NPAGE的大小
...
/*
**页码与第一哈希表中一个相关联的块在wal-index平常小。
**这样就存在一个完整的哈希表和每一个页面大小为32KB的wal-index.对应。

*/
#define HASHTABLE_NPAGE_ONE  (HASHTABLE_NPAGE - (WALINDEX_HDR_SIZE/sizeof(u32)))

/* The wal-index is divided into pages of WALINDEX_PGSZ bytes each. */
/*wal-index日志索引被划分为WALINDEX_PGSZ来记录 */
...
** then an SQLite error code is returned and *ppPage is set to 0.如果这个函数调用成功，ppPage 等于日志索引页并且返回SQLITE_OK。
发生错误，返回 SQLite 错误代码，此时 pppage 等于 0
*/
...
/*
** Return a pointer to the WalCkptInfo structure in the wal-index.返回一个WalCKptINfo结构指针
*/
...
** initial values of 0 and 0 if aIn==NULL).对一个8位字节的校验是基于数组aIn[0] and aIn[1]的初始值
**
** The checksum is written back into aOut[] before returning. 校验结果在返回之前写回在aOut[]
**
** nByte must be a positive multiple of 8.  nbyte 必须是8的整数倍
*/
static void walChecksumBytes(
  int nativeCksum, /* True for native byte-order, false for non-native */
  u8 *a,           /* Content to be checksummed */ 定义校验 内容
...
/*
** Write the header information in pWal->hdr into the wal-index.将pWal->hdr中的标题信息写入wal-index
**
** The checksum on pWal->hdr is updated before it is written. pWal ->hdr 的校验和在被写之前要更新
*/
static void walIndexWriteHdr(Wal *pWal){
  ...
  walShmBarrier(pWal);              调用  walShmBarrier（）函数
...
** 这个功能是编码单一帧头，将其写入到由调用者提供的缓冲区，由下列组成
**     0: Page number.
**     4: For commit records, the size of the database image in pages 
**        after the commit. For all other records, zero.
...
static void walEncodeFrame(
  ...
  u32 iPage,                      /* Database page number for frame */  某一帧在数据库中在哪一页
...
/*
** Check to see if the frame with header in aFrame[] and content检查aFrame【】和adata[] 是否正确，如果正确，填写*piPage  pnTruncate
** in aData[] is valid.  If it is a valid frame, fill *piPage and并且指针return true
** *pnTruncate and return true.  Return if the frame is not valid.
*/
static int walDecodeFrame(
  Wal *pWal,                      /* The write-ahead log */   
  u32 *piPage,                    /* OUT: Database page number for frame */  定义数据库页码
  ...
  */
  ...
  /* A frame is only valid if the page number is creater than zero.只有在页码数大于0时帧才是有效的
  */
...
** a part of an ordinary build.      获取Wal锁的名字  这通常通过传入的参数 lockIdx 的值进行比较 返回锁名
*/
...
** A lock cannot be moved directly between shared and exclusive - it must go一个锁不能直接在共享和胡扯状态之间移动它 必须进入解锁状态
...
** are numbered starting from 0.   返回的wal-index页面包含哈希表和页码数组包含条目对应于WAL框架iFrame。wal-index分为32 kb的页面。Wal-index页面从0开始编号。
*/
...
/*
** Return the page number associated with frame iFrame in this WAL. 返回与IFrame对应的页的数
*/
static u32 walFramePgno(Wal *pWal, u32 iFrame){
  int iHash = walFramePage(iFrame); // 调用walFramePage函数获取与Iframe对应的 索引页在第几页
...
/*
** Remove entries from the hash table that point to WAL slots greater 从哈希表中删除比pWal->hdr.mxFrame更大的相应条目
...
static void walCleanupHash(Wal *pWal){
  ...
  assert( pWal->writeLock ); 判断Wal是否在写事务中，在的话终止程序
  ...
  **获取包含哈希表和页码的指针数组的条目对应帧pWal - > hdr.mxFrame。
  **这是保证页面上述哈希表和数组驻留在已经映射。
  */
  ...
  ** than pWal->hdr.mxFrame.
  */
  iLimit = pWal->hdr.mxFrame - iZero; 获取ilimit的值
...
static int walIndexAppend(Wal *pWal, u32 iFrame, u32 iPage){
  ...
  */
  if( rc==SQLITE_OK ){                   如果调用函数成功 
    ...
    **如果在aPgno[]的条目已经设置，则先前的writer一定会在事务处理过程中意外退出
    **(通过写一个或者多个脏页WAL以释放内存)
    **在编写任何新的条目之前需要先从哈希表中删除writers未提交的残留事务。
    */
    ...
    aHash[iKey] = (ht_slot)idx;  第ikey的hash值为idx
    ...
    ** iteration.　验证每个条目映射区域是可获得的通过哈希表这被证明是一个非常非常重要的检查,所以只是偶尔这样做——而不是在每一个迭代。
    */
    if( (idx&0x3ff)==0 ){  
...
**通过阅读写前日志文件恢复wal- wal-index
**这个程序首先尝试在wal-index创建一个排他锁阻止其他线程去做其他操作当WAL或wal-index在恢复运行时
**。如果无法建立必要的锁,这个例程返回SQLITE_BUSY。
*/
static int walIndexRecover(Wal *pWal){  返回是否加锁进行恢复，不成功返回 SQLite——busy
  ...
  ** this function returns./ * 获取排它锁的所有字节在不被调用者锁定的锁的范围内 
  **调用者是用来保证锁上了WAL_WRITE_LOCK字节,也可能也锁定WAL_CKPT_LOCK的字节
  **如果成功,则之前没有被锁定的字节也将会锁定
  */
  assert( pWal->ckptLock==1 || pWal->ckptLock==0 );如果锁类型返回值为1或者0 ，则终止
  ...
  if( nSize>WAL_HDRSIZE ){            nSize 为32
    ...
    ** WAL file.如果数据库页面大小不是一个两个,或者大于SQLITE_MAX_PAGE_SIZE,则可以知道WAL文件内容是无效的数据。
    **类似地,如果‘魔法’值是无效的,忽略了整体WAL文件。
    */
    ...
    /* Verify that the WAL header checksum is correct */ 核实Wal的头数据的检验是否是正确的
...
finished:                      goto 的标记
  if( rc==SQLITE_OK ){                  r如果 rc 为 Sqlite——ok
    ...
    ** checkpointing the log file. 如果不止一个框架从日志文件中恢复过来，通过sqlite3_log()报告一个事件。
    **这将帮助识别造成在没有检查指示日志文件的情况下应用程序经常关闭的性能问题。
    **
    */
    if( pWal->hdr.nPage ){            
...
**打开一个连接文件zWalName的wal。数据库文件必须已经打开 pDbFd连接。
**zWalName点的缓冲区必须保持有效的生命周期内返回Wal 处理。
...
**当这个函数被调用时一个共享锁应该由数据库文件所持有。
**这个共享锁的目的是防止任何其他客户解除WAL或wal-index文件的链接。
**如果另一个进程也这样做当这个客户端打开这些文件之一时 系统将严重破坏。
** If the log file is successfully opened, SQLITE_OK is returned and 
** *ppWal is set to point to a new WAL handle. If an error occurs,
** an SQLite error code is returned and *ppWal is left unmodified.
**如果成功打开日志文件,将返回SQLITE_OK，ppWal被设置为指向一个新的WAL处理。
**如果出现错误, 将会返回 一个SQLite错误代码和* ppWal修改的。
*/
int sqlite3WalOpen( 
  ...
  const char *zWalName,           /* Name of the WAL file */   定义Wal文件的名
  ...
  **在源文件之前融合os_unix。c和os_win。确认#锁定字节偏移量的定义在os_unix。c和os_win。c同意WALINDEX_LOCK_OFFSET值。
  */
#ifdef WIN_SHM_BASE
...
** number larger than , then return 1.//成功返回0。如果在WAL上没有页面数大于*piPage,然后返回1。
*/
static int walIteratorNext(
  ...
  u32 iMin;                     /* Result pgno must be greater than iMin */ 返回 pgno 一定要比iMin 大
  u32 iRet = 0xFFFFFFFF;        /* 0xffffffff is never a valid page number */ 0xffffffff 不是一个有效的 页数
...
static void walMerge(                  
  ...
  int nLeft,                      /* IN: Elements in array *paLeft */  左链表的元素
...
** the smaller.
**保留aList[X] 和 aList[Y]中最大的值，丢弃最小的值。
*/
static void walMergesort( wal的归并
 ...
 s                   /* IN/OUT: Number of elements in aList[] */数目
  ...
  struct Sublist aSub[13];        /* Array of sub-lists */ 排列sub-lists *
...
#ifdef SQLITE_DEBUG     定义 SQLITE_DEBUG 
  {
    int i;
    for(i=1; i<*pnList; i++){
 ...
 ** lock. 构建一个WalInterator 对象 ，它可以对wal文件中按升序排列的页进行循环操作 。调用者一定要由 checkpoint锁持有   
 **
** On success, make *pp point to the newly allocated WalInterator object 如果成功,使*页指向新WalInterator分配对象并返回SQLITE_OK
** return SQLITE_OK. Otherwise, return an error code. If this routine 否则，返回则返回错误代码，如果这个程序返回的是错误的。**p 的值就不确定了
** returns an error, the value of *pp is undefined.
**
** The calling routine should invoke walIteratorFree() to destroy the
** WalIterator object when it has finished with it.当它完成时调用程序应该调用walIteratorFree()来销毁 WalIterator对象
*/
static int walIteratorInit(Wal *pWal, WalIterator **pp){ 
  WalIterator *p;                 /* Return value */  返回值
  int nSegment;                   /* Number of segments to merge */ 定义有几个段来合并
  u32 iLast;                      /* Last frame in log */ 日志中的 最后的帧
  int nByte;                      /* Number of bytes to allocate */ 分配几个字节
  int i;                          /* Iterator variable */ 声明 迭代变量
...
** lock is successfully obtained or the busy-handler returns 0.试图获得排他WAL锁lockIdx和定义的参数n。
** busy-handler
**功能锁直到调用它并重试成功获得锁或busy-handler返回0。
**如果尝试失败和参数xBusy不为空，那么它是一个忙着处理的函数。
**调用它，然后重试锁，直到锁成功获得或busy-handler返回0
*/
static int walBusyLock(        试图获取Wal的锁根据LockIdex 和     
  Wal *pWal,                      /* WAL connection */
  int (*xBusy)(void*),            /* Function to call when busy */当忙碌时调用该函数
  void *pBusyArg,                 /* Context argument for xBusyHandler */
  int lockIdx,                    /* Offset of first byte to lock */锁定偏移的第一字节
  int n                           /* Number of bytes to lock */定义锁的字节数
){
  int rc;          定义返回值
  do {
...
** 从WAL中复制尽可能多的内容到数据库文件中来相应sqlite3_wal_checkpoint()请求 
**
** The amount of information copies from WAL to database might be limited
** by active readers.  This routine will never overwrite a database page
** that a concurrent reader might be using.
**从WAL数据库复制的的信息量对活跃的读者可能是有限制的。
**这个程序将永远不会覆盖并发读者可能会使用的数据库页面
**
** All I/O barrier operations (a.k.a fsyncs) occur in this routine when在 SQLite
...
**（又名fsyncs）发生在该程序时，在SQLite的
** 在SQLite是在WAL-mode同步时这个程序的所有I / O屏障操作都会发生
**这意味着,如果检查点总是由一个后台线程或后台进程运行，前台线程不会阻塞在冗长的fsync调用。

** Fsync is called on the WAL before writing content out of the WAL and
** into the database.  This ensures that if the new content is persistent
** in the WAL and can be recovered following a power-loss or hard reset.
** 将内容写入到数据库之前将会调用Fsync 
**在断电或重启启后可以保证新的内容持续存在WAL
**
** Fsync is also called on the database file if (and only if) the entire
** WAL content is copied into the database file.  This second fsync makes
** it safe to delete the WAL since the new content will persist in the
** database file.
**数据库文件当且仅当整个WAL内容复制到数据库文件时Fsync会被调用
**这第二个fsync使它可以安全的删除WAL因为新内容将一直存在数据库文件。
**
** This routine uses and updates the nBackfill field of the wal-index header.//这个程序使用和更新nBackfill wal-index头数据
** This is the only routine tha will increase the value of nBackfill.  //这是唯一的程序将增加nBackfill的值
** (A WAL reset or recovery will revert nBackfill to zero, but not increase
** its value.)//重置或者恢复WAL将会使 nBackfill恢复到0状态，但是不会增加它的值
**
** The caller must be holding sufficient locks to ensure that no other
** checkpoint is running (in any other thread or process) at the same
** time. 
**这个调用者必须要有大量的锁来持有，以保证没有其他的checkpoint在同一时刻运行。
*/
static int walCheckpoint(
  ...
  szPage = walPagesize(pWal); 调用函数来获取数据页的大小
  testcase( szPage<=32768 );      测试
  testcase( szPage>=65536 );      测试
  pInfo = walCkptInfo(pWal); 调用函数获取检验的信息
  if( pInfo->nBackfill>=pWal->hdr.mxFrame ) return SQLITE_OK; 如果真 则返回 SQLITE_OK

  /* Allocate the iterator */ 分配 迭代器
  ...
  ** cannot be backfilled from the WAL.　计算在mxSafeFrame指数的最后一帧安全写入数据库。框架之外mxSafeFrame可能覆盖数据库页面所使用的活跃的读者,因此无法回填在wal
  */
  ...
  if( pInfo->nBackfill<mxSafeFrame
   && (rc = walBusyLock(pWal, xBusy, pBusyArg, WAL_READ_LOCK(0), 1))==SQLITE_OK      判断语句
    ...
    if( sync_flags ){   判断是否同步
      rc = sqlite3OsSync(pWal->pWalFd, sync_flags); 调用同步函数
    }

    /* If the database file may grow as a result of this checkpoint, hint
    ** about the eventual size of the db file to the VFS layer. 
    **如果数据库文件可能因为checkpoint扩展
    **暗示关于db文件的最终大小VFS层。
    */
    if( rc==SQLITE_OK ){  如果调用成功
      i64 nReq = ((i64)mxPage * szPage); 定义变量大小
  ...
  **如果这是一个SQLITE_CHECKPOINT_RESTART操作,而且整个文件已复制到数据库文件,然后阻止,直到所有读者使用wal文件已经完成。
  **
  */
...
** it to exactly nMax bytes. If an error occurs while doing so, ignore it. 如果Wal文件大于最大值，缩短它到正确的长度。
*/
static void walLimitSize(Wal *pWal, i64 nMax){
  ...
  if( rx ){       如果rx为真   
    sqlite3_log(rx, "cannot limit WAL size: %s", pWal->zWalName);如果日志已经被激活，将日志信息写入到日志中，
  }
}
...
int sqlite3WalClose(
  ...
  int sync_flags,                 /* Flags to pass to OsSync() (or 0) */  定义同步的标志
  int nBuf,
  u8 *zBuf                        /* Buffer of at least nBuf bytes */ 定义至少有多大字节的缓冲区
){
  int rc = SQLITE_OK;             
  if( pWal ){                         如果wal不为空
...
** a checksum on the header.    wal-index在共享内存。另一个线程或进程在写头数据的同时这个程序可能正在读它,这可能会导致不一致。
**检测到脏读通过验证这两个副本的读数据都是一样的.
** If and only if the read is consistent and the header is different from
** pWal->hdr, then pWal->hdr is updated to the content of the new header
** and *pChanged is set to 1. 当且仅当读数据是一样的，头数据和Pwal->hdr不同的,新头数据的内容进行更新 。 把*pChanged的值设为1
**
...
static int walIndexReadHdr(Wal *pWal, int *pChanged){
  ...
  **如果日志索引的第一页已经映射 试着立即读 wal-index头数据,没有持有任何锁。
  **这通常的工作,但是可能会失败如果wal-index头目前被占用或被另一个线程或进程修改
  */
  ...
  */
  ...
  if( badHdr ){  如果读取失败
    if( pWal->readOnly & WAL_SHM_RDONLY ){
      ...
      if( SQLITE_OK==(rc = walIndexPage(pWal, 0, &page0)) ){如果 获取索引页成功
...
**尝试开始一个读事务，这可能由于竞争或其他突发情况导致失败 如果失败返回 Wal_RETRY 提示 立即重试。
** On success return SQLITE_OK.  On a permanent failure (such an
** I/O error or an SQLITE_BUSY because another process is running
** recovery) return a positive error code.
**　成功返回SQLITE_OK。(这是一个永久失败I / O错误或SQLITE_BUSY因为另一个进程正在运行恢复)返回一个正误差。
...
**useWal参数是真的强迫使用WAL和禁用的WAL忽略,因为它已经完全设置检查点。如果useWal = = 0,这个例程调用walIndexReadHdr()复制wal-index头到pWal - > hdr。如果wal-index头已经改变,pChanged设置为1(就像一个想法给调用者,当地的佩吉特缓存过时的和需要刷新)。useWal = = 1时,wal-index头已经假定被加载和pChanged参数是未使用的。
...
static int walTryBeginRead(Wal *pWal, int *pChanged, int useWal, int cnt){
  ...
  */
  ...
  if( !useWal ){
    ...
    if( rc==SQLITE_BUSY ){
      ...
      **如果在另一个线程或者进程没有恢复运行，WAL_RETRY将转为BUSY错误
      */
      if( pWal->apWiData[0]==0 ){
        ...
        **这个分支时采取的xShmMap（）方法返回SQLITE_BUSY。
        **我们假设这是一个短暂的状态，因此返回WAL_RETRY。
        */
        rc = WAL_RETRY;
      }else if( SQLITE_OK==(rc = walLockShared(pWal, WAL_RECOVER_LOCK)) ){//获取共享锁
        walUnlockShared(pWal, WAL_RECOVER_LOCK);//释放共享锁
        rc = WAL_RETRY;//返回WAL_RETRY
    ...
    */
    ...
    if( rc==SQLITE_OK ){
      if( memcmp((void *)walIndexHdr(pWal), &pWal->hdr, sizeof(WalIndexHdr)) ){
        ...
        **在获得READ_LOCK(0)之前如果框架已经追加到日历这样让阅读器继续在这里是不安全的
        **如果持有READ_LOCK(0)阅读器将会忽略整个日志文件，这意味着数据库文件包含着一个可靠的快照
        **这通常是正确的因为READ_LOCK(0)组织checkpoint的发生。
        **
  ...
  **如果我们迄今为止得到这个，它意味着阅读器将要使用WAL从最近的提交中获得最新内容。
  **现在的工作是选择最接近aReadMark[]项中的一个但不超过pWal-> hdr.mxFrame并锁定该条目
  */
  mxReadMark = 0;
  mxI = 0;
  for(i=1; i<WAL_NREADER; i++){
...
**这个程序通常被SQLite3OpenSnapshot（）函数调用原因：
**需要的快照WAL和wal-index当前的即时状态，当前线程继续使用抓拍技术。其他的线程可能添加新的内容到Wal和Wal-index，但当前线程不考虑它。
** If the database contents have changes since the previous read
** transaction, then *pChanged is set to 1 before returning.  The
** Pager layer will use this to know that is cache is stale and
** needs to be flushed.　如果数据库内容变化是由于从以前读事务,然后*pChanged返回之前被设置为1。需要刷新。
*/
int sqlite3WalBeginReadTransaction(Wal *pWal, int *pChanged){
  int rc;                         /* Return code */ 返回码
  int cnt = 0;                    /* Number of TryBeginRead attempts */ 重新读 的次数

  do{
    rc = walTryBeginRead(pWal, pChanged, 0, ++cnt);  调用函数
  }while( rc==WAL_RETRY ); 当 读取成功
...
** current read transaction is configured to use the WAL.  从在WAL读取一个页面,如果它存在于WAL如果当前读事务配置使用Wal
** 
** The *pInWal is set to 1 if the requested page is in the WAL and          
** has been loaded.  Or *pInWal is set to 0 if the page was not in 
** the WAL and needs to be read out of the database.*pInWal 赋值为1  当需要的page 在Wal中，且已被加载赋值为0 如果 不在wal中，需要充数据库中加载
*/
int sqlite3WalRead(
  Wal *pWal,                      /* WAL handle */ 第一指针
  Pgno pgno,                      /* Database page number to read data for */ 数据页号
  int *pInWal,                    /* OUT: True if data is read from WAL */ 数据是从Wal中读取则为真
  int nOut,                       /* Size of buffer pOut in bytes */ 输出字节流的大小  字节为单位
  u8 *pOut                        /* Buffer to write page data to */写数据的缓冲区
){
  u32 iRead = 0;                  /* If !=0, WAL frame to return data from */
  u32 iLast = pWal->hdr.mxFrame;  /* Last page in WAL for this reader */Wal 最新页
  int iHash;                      /* Used to loop through N hash tables */  哈希表
  ...
  ** result obtained using the hash indexes above.  */如果代价大的的assert()语句可以使用,进行线性搜索wal-index文件的内容。确保结果同意结果上面使用的散列索引
  {
    u32 iRead2 = 0;
    u32 iTest; 
    for(iTest=iLast; iTest>0; iTest--){  循环iLast 
      if( walFramePgno(pWal, iTest)==pgno ){  如果 查询到与相匹配的 
        iRead2 = iTest;
        break;                                 则 跳出循环
      }
    }
...
int sqlite3WalBeginWriteTransaction(Wal *pWal){
  ...
  ** transaction. */ 写事务开启前必须读事务在运行
  assert( pWal->readLock>=0 ); 如果有锁

  if( pWal->readOnly ){ 如果不是只读
  ...
  */
  rc = walLockExclusive(pWal, WAL_WRITE_LOCK, 1); 加锁
  if( rc ){
    return rc;
  }
  pWal->writeLock = 1; 赋值
...
** routine merely releases the lock.写事务结束。提交已经完成。这程序是释放锁
*/
...
/*
** 如果有任何数据被写入（但不提交）到日志文件中，该函数移动写指针返回到事务的开始。并且错误代码返回给调用者。否则，如果回调函数不返回一个错误，这个函数返回SQLITE_OK。
...
int sqlite3WalUndo(Wal *pWal, int (*xUndo)(void *, Pgno), void *pUndoCtx){
  ...
  if( ALWAYS(pWal->writeLock) ){ 如果pWal->writeLock是否为真， 
    ...
    ** was in before the client began writing to the database.  恢复WAL索引头的状态，iframe是在客户端开始写入到数据库之前的客户端缓存。
    */
...
** walWriteToLog(). 当前Wal文件所处状态和何处发生同步通过sqlite3WalFrames()转化成 walWriteToLog()的信息。
*/
typedef struct WalWriter {
  Wal *pWal;                   /* The complete WAL information */ 定义Wal 指针变量
...
static int walWriteToLog(
  ...
  if( iOffset<p->iSyncPoint && iOffset+iAmt>=p->iSyncPoint ){                                    P->ISyncPoint 是否在偏移和偏移后之间
    int iFirstAmt = (int)(p->iSyncPoint - iOffset); 计算第一次偏移量
...
static int walWriteOneFrame(
  WalWriter *p,               /* Where to write the frame */ 定义WalWrite 指针
  PgHdr *pPage,               /* The page of the frame to be written */ 要写入的帧页
  int nTruncate,              /* The commit flag.  Usually 0.  >0 for commit */ 提交标记
  sqlite3_int64 iOffset       /* Byte offset at which to write */ 偏移字节
){
  int rc;                         /* Result code from subfunctions */从子函数返回
  void *pData;                    /* Data actually written */实际要写数据
  u8 aFrame[WAL_FRAME_HDRSIZE];   /* Buffer to assemble frame-header in */帧头数据要编译的缓冲区
  ...
  rc = walWriteToLog(p, aFrame, sizeof(aFrame), iOffset); 调用函数写入到Wal中。
  if( rc ) return rc;
  /* Write the page data */ 编写页面数据
  rc = walWriteToLog(p, pData, p->szPage, iOffset+sizeof(aFrame));调用函数写入到页中
  return rc;
}
...
int sqlite3WalFrames(
  Wal *pWal,                      /* Wal handle to write to */ 定义变量
  int szPage,                     /* Database page-size in bytes */ 数据库页的大小，单位是字节
  PgHdr *pList,                   /* List of dirty pages to write */  脏页写的列表
  ...
  PgHdr *pLast = 0;               /* Last frame in list */ 在链表中最后一帧
  int nExtra = 0;                 /* Number of extra copies of last page */ 
  int szFrame;                    /* The size of a single frame */ 单帧的大小
  i64 iOffset;                    /* Next byte to write in WAL file */ 偏移字节
  WalWriter w;                    /* The writer */ WalW的变量

  assert( pList );判断链表是否为空，为空则终止程序
  assert( pWal->writeLock ); 判断是加锁

  /* If this frame set completes a transaction, then nTruncate>0.  If
  ** nTruncate==0 then this frame set does not complete the transaction. */
  assert( (isCommit!=0)==(nTruncate!=0) ); 如果提交为真，Wal不为0 
  ...
  */如果这是第一帧写入日志,Wal头数据写在WAL文件。看部这个源文件的描述WAL标题格式
  ...
  w.szPage = szPage;
  iOffset = walFrameOffset(iFrame+1, szPage); 计算偏移量
  szFrame = szPage + WAL_FRAME_HDRSIZE; 

  /* Write all frames into the log file exactly once */一次精确的把所有帧写入日志文件
  for(p=pList; p; p=p->pDirty){ 对链表进行遍历
    int nDbSize;   /* 0 normally.  Positive == commit flag */ 正常位0 ，正数为commit flag
    iFrame++; 
    assert( iOffset==walFrameOffset(iFrame, szPage) ); 如果偏移量为
    nDbSize = (isCommit && p->pDirty==0) ? nTruncate : 0; 如果提交且P的脏数据标记为0 则 返回nTruncate，否则为0
  ...
  **如果这事务的结束,那么我们可能需要填补事务和/或同步WAL文件
  ...
  if( isCommit && (sync_flags & WAL_SYNC_TRANSACTIONS)!=0 ){ 
    if( pWal->padToSectorBoundary ){
      ...
      while( iOffset<w.iSyncPoint ){
        rc = walWriteOneFrame(&w, pLast, nTruncate, iOffset);调用函数写入到OneFrame
        if( rc ) return rc;
        iOffset += szFrame;
        nExtra++;
      }
  ...
  */将数据添加到wal-index。没有必要锁定wal-index为此wal-index SQLITE_SHM_WRITE锁来保证没有其它写操作,和没有数据被现有的读操作使用且被覆盖。
  iFrame = pWal->hdr.mxFrame;
  for(p=pList; p && rc==SQLITE_OK; p=p->pDirty){   对链表进行遍历
    iFrame++;
    rc = walIndexAppend(pWal, iFrame, p->pgno); 调用函数
  }
  while( rc==SQLITE_OK && nExtra>0 ){ 当调用成功
    iFrame++;
    nExtra--;
...
int sqlite3WalCheckpoint(
  ...
  int sync_flags,                 /* Flags to sync db file with (or 0) */同步标志
  int nBuf,                       /* Size of temporary buffer */临时缓冲区的大小
  u8 *zBuf,                       /* Temporary buffer to use */ 临时缓存区
  int *pnLog,                     /* OUT: Number of frames in WAL */在Wal中的个数
  int *pnCkpt                     /* OUT: Number of backfilled frames in WAL */在WAL中回填的个数
){
  int rc;                         /* Return code */ 返回码
  ...
  rc = walLockExclusive(pWal, WAL_CKPT_LOCK, 1); 加锁
  if( rc ){
    /* Usually this is SQLITE_BUSY meaning that another thread or process
    ** is already running a checkpoint, or maybe a recovery.  But it might
    ** also be SQLITE_IOERR. */ 一般来说SQLITE_BUSY意味着 另一个线程或进程已经在运行checkpoint或则进行恢复。 也可能是SQLITE_IOERR
    return rc;
  }
  pWal->ckptLock = 1; 为ckptLock赋值

  /* If this is a blocking-checkpoint, then obtain the write-lock as well
  ** to prevent any writers from running while the checkpoint is underway.
  ** This has to be done before the call to walIndexReadHdr() below.如果这是一个blocking-checkpoint,然后获得写锁，以防止任何写检查点正在运行。　　* *必须贼调用walIndexReadHdr()之前做。
  **
  ** If the writer lock cannot be obtained, then a passive checkpoint is
  ** run instead. Since the checkpointer is not holding the writer lock,
  ** there is no point in blocking waiting for any readers. Assuming no 
  ** other error occurs, this function will return SQLITE_BUSY to the caller.
  */如果无法获得写锁,那么一个被动的检查点将代替运行。由于checkpoint无法加载锁，这是毫无意义的阻塞等待任何读者，假设没有其他错误发生时,该函数将返回SQLITE_BUSY给调用者。
  if( eMode!=SQLITE_CHECKPOINT_PASSIVE ){  如果模式不是SQLITE_CHECKPOINT_PASSIVE 
    rc = walBusyLock(pWal, xBusy, pBusyArg, WAL_WRITE_LOCK, 1); 获取锁
    if( rc==SQLITE_OK ){ 如果调用成功
      pWal->writeLock = 1;  更改参数值
    }else if( rc==SQLITE_BUSY ){ 如果返回时Sqlite_busy
      eMode2 = SQLITE_CHECKPOINT_PASSIVE;  更改它的值
      rc = SQLITE_OK;
    }
  }
...
**返回的值传递给sqlite3_wal_hook回调中，
**在WAL帧中的最后的数量正要提交
**因为 sqlite3WalCallback（）被调用。
*/
...
**
**这个函数被调用来改变WAL子系统流入或流出
**当locking_mode的**= EXCLUSIVE。
...
int sqlite3WalExclusiveMode(Wal *pWal, int op){
  ...
  */
  ...
  if( op==0 ){//如果op==0  将locking_mode=NORMAL 改变为locking_mode=EXCLUSIVE
    if( pWal->exclusiveMode ){
      ...
      /* Already in locking_mode=NORMAL *///如果locking_mode=NORMAL 返回值为0
      rc = 0;
    }
  }else if( op>0 ){
