...
/*
这个文件使用来实现外部(基于磁盘)数据库
开头看见"btreeInt.h"文件里附加一些信息 
包括文件格式的描述和操作的概述。
*/
#include "btreeInt.h"
...
/*
定义一个常字符串，然后将"btreeInt.h"里的定义的一个sqlite头文件的值赋给这个常字符串
...
*/
/*
这是一个宏指令，它将全局变量设置为1，如果是0，就定义一个整形的追踪树，
并赋值为1；然后定义一个追踪函数，如果sqlite3BtreeTrace为真，就进行追踪。
否则一开始就为真的话，就进行追踪。
*/
#if 0
int sqlite3BtreeTrace=1;  /* True to enable tracing */ //是true，则追踪
...
*/
/*
**从一个无符号字节数组中提取保存cell的地址的大端整数，但如果这个值为0，就将它赋值为65536。
...
*/
/*
** btree结构中最主要包含一个BtShared结构，该结构有权限访问共享缓存，
这个变量构建时有一个文件作用域，但是测试工具需要访问这个变量，因此
我们为了测试就把这个变量变成全局变量。访问这个变量时，会受SQLITE_MUTEX_STATIC_MASTER保护
*/
#ifdef SQLITE_TEST
...
*/
/*
**启动或禁止共享页和模式特征，这个程序没有影响到现存的数据库的连接，
...
*/
/*
这个函数仅仅是作为一个assert()语句的一部分。
检查pBtree拥有所需的锁读或写iRoot与根表页面。如果不返回1如果真的和0。
例如,当写入表根页iRoot通过Btree连接pBtree:
assert(hasSharedCacheTableLock(WRITE_LOCK pBtree iRoot 0));
当编写一个索引,驻留在共享数据库,
调用者应该首先获得一个锁指定相应的根页表。
这使得事情更加复杂,因为这个模块对每个表作为一个单独的结构。
确定写入表的索引,这个函数搜索数据库模式。
而不是根植在页面iRoot上的表或者索引上的锁,调用者
在举行一个写锁模式表(根1页)。这也是可以接受的。
*/
static int hasSharedCacheTableLock(
  Btree *pBtree,         /* Handle that must hold lock */ /*处理必须持有锁*/
  Pgno iRoot,            /* Root page of b-tree */ /*此Btree的根页页号*/
  int isIndex,           /* True if iRoot is the root of an index b-tree *//*索引B树的根页*/
  int eLockType          /* Required lock type (READ_LOCK or WRITE_LOCK) */ /*需要锁类型（读锁或是写锁）*/
){
  ...
  */
  /*
如果客户端是读和写一个索引模式并不是加载,
那么它实际上是很难检查正确的锁。因此不要担心,返回true就行。
这种情况并不经常出现。
  */
  ...
  ** read-lock on iTab will suffice. Return 1 if any of these are found.  */
  /*
  寻找所需的锁（pLock）。在根页iTab上的写锁(pLock->eLock==WRITE_LOCK) ，
  ...
  */
  ...
  /* Failed to find the required lock. *//*没有找到所需的锁,则返回0*/
  return 0;
}
#endif /* SQLITE_DEBUG */
...
*/
/*
可以使用这个函数只assert()语句的一部分。如果是因为其他共享连接同时读取同一个表或索引，
...
*/
/*
查询，判断B树句柄p是否能在iTab根页的表上获取eLock类型的锁（读锁或者是写锁）
...
static int setSharedCacheTableLock(Btree *p, Pgno iTable, u8 eLock){
  ...
  ** table, and that lock is obtained in BtreeBeginTrans().  */
  /*
  有读未提交标志的永远不会尝试使用这个函数获得读锁。
  唯一通过连接见sqlite_master表读未提交模式,
  ...
  */
  /*
  如果上面的搜索没有找到一个( !pLock)BtLock结构关联的在iTable上的B树p，
  分配一个( pLock = (BtLock *)sqlite3MallocZero(sizeof(BtLock)); )并把它链接到列表中。
  */
  if( !pLock ){
  ...
  */
  /*
设置BtLock.eLock变量为当前的锁与所请求的锁的最大值
...
*/
/*
释放所有的表锁(锁获得通过调用setSharedCacheTableLock()过程)Btree对象持有的p。
此函数假定B树P有一个开放的读或写操作事务。
如果没有，那么BTS_PENDING标志可能被不正确地清除。
*/
static void clearAllSharedCacheTableLocks(Btree *p){
  ...
  if( pBt->pWriter==p ){
    ...
    */
    /*
	在B树p结束事务时，将调用此函数。如果目前存在一个写事务，
	并且p不是那个写事务。那么连接进程而不是
	写进程持有的锁的数量大约降至零。
	在这种情况下，设置BTS_PENDING标志为0。
	如果目前还没有一个写事务，那么BTS_PENDING必须为零。
	因此，下一行(pBt->btsFlags &= ~BTS_PENDING;)在这种情况下是没有影响的。
	*/
    pBt->btsFlags &= ~BTS_PENDING;
  }
}
...
static void releasePage(MemPage *pPage);  /* Forward reference */ //释放
...
*/
/*
这个程序用于内部的assert()仅仅确认游标BtShared持有互斥锁
*/
#ifdef SQLITE_DEBUG
...
*/
/*
在共享B树结构pBt上，所有打开的游标上的溢出页无效。invalidateOverflowCache(p)
*/
...
/*
这个函数被调用之前修改一个表的内容
无效的任何incrblob游标打开行或行被修改。
如果论点isClearTable是真的,那么的全部内容
表将被删除。在这种情况下所有incrblob失效
游标打开表中的所有行根页pgnoRoot。
否则,如果论点isClearTable是假的,那么行
rowid iRow被替换或删除。在这种情况下无效
只有那些incrblob游标打开特定行。
*/
static void invalidateIncrblobCursors(
  Btree *pBtree,          /* The database file to check */         /*数据库文件来检查*/
  i64 iRow,               /* The rowid that might be changing */   /*rowid可能发生变化*/
  int isClearTable        /* True if all rows are being deleted */ /*如果所有行被删除,则True*/
){
  BtCursor *p;
...
#else
  /* Stub functions when INCRBLOB is omitted */  /*当INCRBLOB被省略时，存根函数*/
...
/*
设置一些使用pgno BtShared。pHasContent bitvec。
当一个页面之前包含数据成为空闲列表的叶子页面,这时被调用。
BtShared。pHasContent bitvec存在在一个不起眼的工作
错误引起的周围两个有用的IO优化之间的交互
叶空闲列表页:
1)当所有数据从一个页面和页面删除
叶空闲列表页面,页面不写入数据库(如叶空闲列表页面不包含有意义的数据)。
有时这样一个页面甚至不是日志(不会被修改,为什么其他的日志?)。
2)当一个空闲列表叶页面重用,其内容不是阅读
从数据库或写入日志文件(为什么它是,如果不是有意义呢?)。
本身这些优化工作,提供一个方便的批量删除或插入操作的性能提升。然而,如果
搬到一个页面内的空闲列表然后重用相同交易,出现问题。如果页面没有杂志时
搬到空闲列表,它也不是杂志时从空闲列表中提取和重用,那么原始数据
可能会丢失。在发生回滚,它可能是不可能的恢复数据库到原来的配置。
解决方案是BtShared。pHasContent bitvec。当一个页面
搬到成为一个空闲列表页,相应的位在bitvec中设置。
每当从空闲列表中提取叶子页面时,优化2如果相应的一些已经被忽略
BtShared.pHasContent。bitvec的内容被清除在每笔交易的结束。
...
这个函数被调用时叶空闲列表页面中
空闲列表以便重用。它检索返回false,如果它是安全的
从页面调度程序层页面设置没有内容的标志。否则为True。
*/
...
static int saveCursorPosition(BtCursor *pCur){
  ...
  assert( CURSOR_VALID==pCur->eState );/*游标有效*/
  assert( 0==pCur->pKey );
  assert( cursorHoldsMutex(pCur) );

  rc = sqlite3BtreeKeySize(pCur, &pCur->nKey);
  assert( rc==SQLITE_OK );  /*返回SQLITE_OK */
  ...
  */
  /*
  如果这是一个intKey表,然后上面的调用BtreeKeySize()
...
*/
/*
保存所有游标的位置(pExcept除外)与打开表上根页的iRoot。
...
/*在这个版本的BtreeMoveto,pKey拥挤指数由OP_MakeRecord操作码生成等记录。
打开记录,然后调用BtreeMovetoUnpacked()来做这个工作。*/
static int btreeMoveto(
  BtCursor *pCur,     /* Cursor open on the btree to be searched */    /*游标打开btree搜索*/
  const void *pKey,   /* Packed key if the btree is an index */        /*如果btree索引是包装的关键*/
  i64 nKey,           /* Integer key for tables.  Size of pKey for indices */  /*表中的整形键*/
  int bias,           /* Bias search to the high end */     /*搜索到高端*/
  int *pRes           /* Write search results here */    /*搜索结果写在这里*/
){
  int rc;                    /* Status code */  /*状态码*/
  UnpackedRecord *pIdxKey;   /* Unpacked index key */  /*打开索引键*/
  char aSpace[150];          /* Temp space for pIdxKey - to avoid a malloc */  /*临时空间*/
  char *pFree = 0;
...
static int btreeRestoreCursorPosition(BtCursor *pCur){
  ...
  if( pCur->eState==CURSOR_FAULT ){
    return pCur->skipNext;  /* Prev() is noop if (skipNext) negative. Next() is noop if positive */  /*如果是Prev()等待则返回负的，如果是Next()等待则返回正的*/
  }
  pCur->eState = CURSOR_INVALID;
...
确定是否一个游标已经从它的位置
最后被放置。游标可以移动,当行指向
在被删除从他们。如果出现错误，这个程序返回一个错误代码。
如果游标已经移动了，则 pHasMoved这个整形指针被设置为1，否则为0。
*/
...
/*
鉴于常规数据库页面的页码,返回的页面
数量pointer-map页面,其中包含的条目
输入页码。返回0(不是一个有效的页面)以来pgno = = 1
没有指针映射与第1页。integrity_check逻辑
要求ptrmapPageno(* 1)! = 1。
*/
...
*写一个进入的指针映项。
**这个程序更新页码'key'的指针映射项
以便它映射到类型'eType'和父页码'pgno'如果*pRC最初非零(non-SQLITE_OK)，则这个程序可以任何操作。
如果发生错误，适当的错误代码会写进*pRC*/
static void ptrmapPut(BtShared *pBt, Pgno key, u8 eType, Pgno parent, int *pRC){
  DbPage *pDbPage;  /* The pointer map page */  /*Pager的页句柄*/
  u8 *pPtrmap;      /* The pointer map data */  /*地图数据的指针*/
  Pgno iPtrmap;     /* The pointer map page number */  /*指针映射页码*/
  int offset;       /* Offset in pointer map page */
  int rc;           /* Return code from subfunctions(子函数) */

  if( *pRC ) return;

  assert( sqlite3_mutex_held(pBt->mutex) );
  /* The master-journal page number must never be used as a pointer map page */ /*master-journal页面数量绝不能被用作一个指针地图页面*/
...
static int ptrmapGet(BtShared *pBt, Pgno key, u8 *pEType, Pgno *pPgno){
  DbPage *pDbPage;   /* The pointer map page */ /*Pager的页句柄*/
  int iPtrmap;       /* Pointer map page index *//*指针映射页面索引*/
  u8 *pPtrmap;       /* Pointer map page data */ /*指针映射页数据*/
  int offset;        /* Offset of entry in pointer map */ /*指针映射的输入偏移*/
  int rc;
...
*/
/*
在btree页面和一个cell index当中 (0意味着页面上的第一个单元格,1意味着第二个单元格,等等)返回一个指针指向单元内容。
...
/*
这个更复杂的版本的findCell()为页面做的工作包含溢出的cells。
*/
...
*/
/*
**解析cell content block，填在CellInfo结构中。这个函数有两个版本。
...
static void btreeParseCellPtr(
  MemPage *pPage,         /* Page containing the cell */  /*包含单元格的页面*/
  u8 *pCell,              /* Pointer to the cell text. */  /*指向单元内容的指针*/
  CellInfo *pInfo         /* Fill in this structure */   /*填充这个结构*/ 
){
  u16 n;                  /* Number bytes in cell content header */ /*单元格内容标题中的字节数*/
  u32 nPayload;           /* Number of bytes of cell payload */   /*信元有效负载的字节数*/ 
  ...
  if( likely(nPayload<=pPage->maxLocal) ){
    ...
    */
    /*
	如果负载将不完全匹配本地页，我们必须决定多少存储在本地和多少泄漏到溢出页。
	未使用的策略是减少空间溢出页同时保持中的本地存储minLocal和maxLocal之间。
	警告:以任何方式改变溢流载荷分布的方式会导致不兼容的文件格式。
	*/
    int minLocal;  /* Minimum amount of payload held locally */    /*本地有效负载的最小数量*/
    int maxLocal;  /* Maximum amount of payload held locally */    /*本地有效负载的最大数量*/
    int surplus;   /* Overflow payload available for local storage */ /*可用于本地存储的溢出有效负载*/
...
static void btreeParseCell(
  MemPage *pPage,         /* Page containing the cell */  /*包含单元格的页面*/
  int iCell,              /* The cell index.  First cell is 0 */ /*单元格索引，首单元格是0*/
...
static u16 cellSizePtr(MemPage *pPage, u8 *pCell){
  ...
  ** this function verifies that this invariant(不变式)is not violated(违反). */
  ...
  /*
  这个函数返回的值应该是相同的（CellInfo.nSize）值做一个全面的解析单元格的发现。
  ...
  */
  ...
  /* The minimum size of any cell is 4 bytes. */  /*任何单元的最小尺寸为4个字节。*/
  if( nSize<4 ){
    nSize = 4;
  }
...
/*这种变化对cellsizeptr()里面assert()语句只能使用。*/
...
*/
/*
如果pCell(pPage的一部分)包含指向溢出页的指针，插入一个入口到pointer-map
*/
...
*/
/*
整理页面。所有的单元格都移到页面结束所有的自由空间被收集到
一个大的FreeBlk发生在头和单元格指针和数组内容区域之间。

*/
static int defragmentPage(MemPage *pPage){
  int i;                     /* Loop counter */   /*循环计数器*/
  int pc;                    /* Address of a i-th cell */   /*一个单元格地址*/
  int hdr;                   /* Offset to the page header */  /*偏移页首*/
  int size;                  /* Size of a cell */   /*单元格的大小*/
  int usableSize;            /* Number of usable bytes on a page */    /*页面上可用字节数*/
  int cellOffset;            /* Offset to the cell pointer array */    /*偏移到单元格指针数组*/
  int cbrk;                  /* Offset to the cell content area */     /*偏移到单元格内容区域*/
  int nCell;                 /* Number of cells on the page */         /*页面上的单元格数*/
  unsigned char *data;       /* The page data */                       /*页面数据*/
  unsigned char *temp;       /* Temp area for cell content */         /*单元格内容的临时区域*/
  int iCellFirst;            /* First allowable cell index */         /*允许单元格索引的第一个*/
  int iCellLast;             /* Last possible cell index */            /*最后一个可能的单元格索引*/
  ...
  for(i=0; i<nCell; i++){
    ...
    */
	/*
	如果sqlite_enable_oversize_cell_check定义,则这些条件已经验证了btreeinitpage()
	*/
    if( pc<iCellFirst || pc>iCellLast ){
      return SQLITE_CORRUPT_BKPT;
    }
#endif
    assert( pc>=iCellFirst && pc<=iCellLast );/*在第一个允许cell索引，最后一个可能的cell索引之间。*/
...
*/
/*
**在pPage上分配nByte字节的空间，将索引写入pIdx中分配空间的第一个字节。
...
static int allocateSpace(MemPage *pPage, int nByte, int *pIdx){
  const int hdr = pPage->hdrOffset;    /* Local cache of pPage->hdrOffset */  /*本地缓存*/
  u8 * const data = pPage->aData;      /* Local cache of pPage->aData */
  int nFrag;                           /* Number of fragmented bytes on pPage */  /*碎片字节数*/
  int top;                             /* First byte of cell content area */      /*单元内容区域的第一个字节*/
  int gap;        /* First byte of gap between cell pointers and cell content */   /*第一个字节的cell指针和cell内容之间的差距*/
  int rc;         /* Integer return code */ /*返回代码为整形*/
  int usableSize; /* Usable size of the page */   /*页面可用大小*/
  
  assert( sqlite3PagerIswriteable(pPage->pDbPage) );
  assert( pPage->pBt );
  assert( sqlite3_mutex_held(pPage->pBt->mutex) );
  assert( nByte>=0 );  /* Minimum cell size is 4 */  /*最小单元尺寸为4*/
  ...
  if( nFrag>=60 ){
    /* Always defragment highly fragmented pages */   /*总是对高度分散的页面*/
...
这里大部分的精力coalesing相邻的空闲块成一个大的空闲块。

*/
static int freeSpace(MemPage *pPage, int start, int size){
  int addr, pbegin, hdr;
  int iLast;                        /* Largest possible freeblock offset */  /*最大的可能空闲偏移*/
  ...
  if( pPage->pBt->btsFlags & BTS_SECURE_DELETE ){
    ...
    ** option is enabled */   /*覆盖删除信息时，secure_delete零点选项启用*/
  ...
  */ 
  ...
  /*
  添加空间回到freeblocks链表。注意，即使freeblock名单是由btreeinitpage()，btreeinitpage()没有检测到cells
  ...
  */
  ...
  /* Coalesce adjacent free blocks */   /*合并相邻空闲块*/
  addr = hdr + 1;
  ...
  /* If the cell content area begins with a freeblock, remove it. */  /*如果单元格内容区域开始是空闲块，则删除它。*/
...
/*解码一页的标记字节（头部的第一个字节）初始化的MemPage相应结构域。
...
static int btreeInitPage(MemPage *pPage){
  ...
  if( !pPage->isInit ){
    ...
    u8 hdr;            /* Offset to beginning of page header */   /*偏移到开始页面的头部*/
    u8 *data;          /* Equal to pPage->aData */
    BtShared *pBt;        /* The main btree structure */
    int usableSize;    /* Amount of usable space on each page */   /*每一页的可用空间量*/
    u16 cellOffset;    /* Offset from start of page to first cell pointer *//*从页面开始到第一个单元格指针的偏移量*/
    int nFree;         /* Number of unused bytes on the page */  /*页上未使用的字节数*/
    int top;           /* First byte of the cell content area */  /*小区内容区域的第一个字节*/
    int iCellFirst;    /* First allowable cell or freeblock offset */  /*第一个允许cell或freeblock偏移*/
    int iCellLast;     /* Last possible cell or freeblock offset */ /*最后可能的cell或freeblock偏移*/
    ...
    if( pPage->nCell>MX_CELL(pBt) ){
      /* To many cells for a single page.  The page must be corrupt */  /*为单页的多个单元格.*/
      return SQLITE_CORRUPT_BKPT;
    }
    ...
    */
	...
	/*
	有缺陷的数据库错误页面可能会为过去的读结束时,进行分析一个单元格。
	...
	*/
...
#if defined(SQLITE_ENABLE_OVERSIZE_CELL_CHECK)
    {
      int i;            /* Index into the cell pointer array */   /*索引到单元格数组的指针*/
      int sz;           /* Size of a cell */  /*设置单元格的大小*/
    ...
    /* Compute the total free space on the page */  /*计算页面上的总空闲空间*/
    pc = get2byte(&data[hdr+1]);
    nFree = data[hdr+7] + top;
    while( pc>0 ){
      u16 next, size;
      if( pc<iCellFirst || pc>iCellLast ){
        /* Start of free block is off the page */   /*启动空闲块是关闭页面*/
        return SQLITE_CORRUPT_BKPT; 
      }
      ...
      if( (next>0 && next<=pc+size+3) || pc+size>usableSize ){
        ...
        ** the free-block must lie on the database page.  */   /*空闲块必须按升序排列。和空闲块的最后一个字节必须位于数据库页*/
        return SQLITE_CORRUPT_BKPT; 
      }
      nFree = nFree + size;
      pc = next;
    ...
    */
	...
	/*
	**在这一点上，nfree包含偏移的总和的内容区域开始加单元格内容范围内可用的字节数。
...
/*convert DbPage into MemPage*/ /*将从pager中获得的DbPage转化为btree中使用的MemPage*/
...
/*
**如果需要，则进行初始化 MemPage.pBt和MemPage.aData的元素
**如果noContent标志设置，这意味着我们不在乎此时的页面内容。
所以不要去磁盘获取内容。只需填写内容与零现在。
**如果将来我们调用sqlite3pagerwrite()这个页面上，
这意味着我们已经开始关注内容和读盘应该发生在那一点。

*/
static int btreeGetPage(
  BtShared *pBt,       /* The btree */
  Pgno pgno,           /* Number of the page to fetch */   /*本页的页号*/
  MemPage **ppPage,    /* Return the page in this parameter */   /*返回此参数中的页*/
  int noContent        /* Do not load page content if true */    /*如果真的,不要加载页面内容*/
){
  int rc;
  DbPage *pDbPage;
...
/*从缓存页检索。如果请求的页不在缓存返回null。如果需要初始化mempage.pbt和mempage.adata元素*/
...
/*返回数据库文件的大小。如果有任何错误,则返回((unsigned int)-1)*/
...
*/
/*
**初始化，这个程序仅仅是一个方便的包装，单独调用btreegetpage()和btreeinitpage()。
**如果出现错误，那么值* pppage将是未定义的。它
可以保持不变，或可能被设置为无效值。
*/
static int getAndInitPage(
  BtShared *pBt,          /* The database file */  /*数据库文件*/
  Pgno pgno,           /* Number of the page to get */  /*获得本页的页号*/
  MemPage **ppPage     /* Write the page pointer here */  /*在这里写入页指针*/
){
  int rc;
  assert( sqlite3_mutex_held(pBt->mutex) );
  ...
  if( pgno>btreePagecount(pBt) ){
    ...
    rc = btreeGetPage(pBt, pgno, ppPage, 0); /*Get a page from the pager*/ /*从pager获取页*/
    if( rc==SQLITE_OK ){
...
*/
/*
**回滚后，页重新装information到cache。因此，在事务开始时，该缓存将恢复到它的原始状态，
对于每一个页面都恢复了这个程序的调用。
  ...
  if( pPage->isInit ){
    ...
    if( sqlite3PagerPageRefcount(pData)>1 ){
      ...
      ** the call for every page that comes in for re-initing. */
		...
		/*页可能不是Btree页；它可能是一个溢出页或ptrmap页或一个空闲的主页。
...
int sqlite3BtreeOpen(
  ...
  const char *zFilename,  /* Name of the file containing the BTree database */   /*含B树数据库的文件名*/
  sqlite3 *db,            /* Associated database handle */   /*关联数据库句柄*/
  Btree **ppBtree,        /* Pointer to new Btree object written here */   /*新的B树对象的指针*/
  int flags,              /* Options */
  int vfsFlags            /* Flags passed through to sqlite3_vfs.xOpen() */
){
  BtShared *pBt = 0;             /* Shared part of btree structure */   /*B树结构的共同部分*/
  Btree *p;                      /* Handle to return */                  /*返回处理*/
  sqlite3_mutex *mutexOpen = 0;  /* Prevents a race condition. Ticket #3537 */   /*防止竞争条件。#3537票*/
  int rc = SQLITE_OK;            /* Result code from this function */    /*此函数的结果代码*/
  u8 nReserve;                   /* Byte of unused space on each page */  /*每个页面上未使用的空间字节*/
  unsigned char zDbHeader[100];  /* Database header content */   /*数据库标题内容*/
  ...
  */
  /*
  设置变量ismemdb真正为一个内存数据库，或虚假的一个基于文件的数据库。
  */
  ...
  assert( (flags&0xff)==flags );   /* flags fit in 8 bits */  /*8位的标志*/

  /* Only a BTREE_SINGLE database can be BTREE_UNORDERED */  /*只有一个btree_single数据库可以是btree_unordered*/
  assert( (flags & BTREE_UNORDERED)==0 || (flags & BTREE_SINGLE)!=0 );

  /* A BTREE_SINGLE database is always a temporary and/or ephemeral */  /*一个btree_single数据库总是暂时的和/或短暂的*/
  ...
  */
/*如果这B树是一个共享缓存的候选,则试图找到一个现有的btshared对象可以让我们分享*/
  if( isTempDb==0 && (isMemdb==0 || (vfsFlags&SQLITE_OPEN_URI)!=0) ){
    if( vfsFlags & SQLITE_OPEN_SHAREDCACHE ){
      ...
      */
/*在调试模式下，我们标记所有持续数据库，即使他们不共享。
提供更多的asserts(sqlite3_mutex_held())语句来找到锁定问题。
*/
      p->sharable = 1;
    }
#endif
  ...
  if( pBt==0 ){
    ...
    */
	  ...
	  /*
	  以下断言确保使用的B树结构正确的尺寸。这是在一个不同的体系结构编译时，对结果的大小变化进行保护。
	  */
    ...
    if( pBt->pageSize<512 || pBt->pageSize>SQLITE_MAX_PAGE_SIZE
      ...
      */
	  /*
	  **如果这个神奇的名字 ":memory:"将创建一个内存数据库，然后把autovacuum模式0（不自动真空），
        即使sqlite_default_autovacuum是真。另一方面，如果sqlite_omit_memorydb已经被定义，
		那么":memory:"只是一个普通的文件名。在这种情况下，自动真空适用于正常。
	  */
    ...
    assert( (pBt->pageSize & 7)==0 );  /* 8-byte alignment of pageSize */   /*8字节对齐的页大小*/
   
#if !defined(SQLITE_OMIT_SHARED_CACHE) && !defined(SQLITE_OMIT_DISKIO)
    /* Add the new BtShared object to the linked list sharable BtShareds.
    */
	/*
	添加新的BtShared对象链表共享BtShareds。
	*/
    if( p->sharable ){
      MUTEX_LOGIC( sqlite3_mutex *mutexShared; )
      pBt->nRef = 1;
  ...
  */
  /*
  如果新的B树使用一个共享的pBtShared，然后链接新的B树的所有共享Btrees列表相同的连接。
  该列表保存在上升的PBT地址顺序。
  */
  if( p->sharable ){
    int i;
...
btree_open_out:
  if( rc!=SQLITE_OK ){
    ...
    */
	  /*
	  如果B树被成功打开，设置缓存大小的默认值。只是，当打开一个现有的共享缓存，
      不改变缓存大小。
	  */
...
*/
/*
BtShared.nRef递减计数器。当它到达零，从共享列表中删除BtShared结构。
...
int sqlite3BtreeClose(Btree *p){
  ...
  /* Close all cursors opened via this handle.  */  /*关闭所有打开的游标句柄。*/
  ...
  */
  /*
  如果仍有共享的B树结构的其他突出的参考，现在回归。这一过程的剩余部分清理共享B树。
  */
...
**缓存页面的最大数量设置为mxpage绝对值。如果mxpage是负的，
分页器将异步操作（不同时）-它会不停的做fsync()确保数据被写入到磁盘表面继续前。
如果同步是关闭的，则事务仍然工作，如果该程序将无法被损坏崩溃。
但是，如果操作系统崩溃或有突然断电时同步时，数据库可能处于不一致的和不可恢复的状态离开。
...
int sqlite3BtreeSetSafetyLevel(
  Btree *p,              /* The btree to set the safety level on */   /*B树要设置安全级别*/
  int level,             /* PRAGMA synchronous.  1=OFF, 2=NORMAL, 3=FULL */  /*语用同步*/
...
*/
/*
如果给定的B树是集安全级别1返回true。换句话说，如果没有sync()发生在磁盘文件返回true。
*/
...
*/
/*
**更改默认的页大小和保留的字节数量每页。或者，如果网页的大小已经固定，sqlite_readonly不做任何改变。
...
*/
/*
在每一页都intentually闲置结束返回的字节数的空间。这是“保留”的空间，有时使用扩展。
*/
...
/*
设置最大页数为数据库如果mxpage是正。没有变化，如果mxpage是0或负数了。不管mxpage的值，返回的最大页数。
...
/*
如果newflag是0或1集的bts_secure_delete标志。如果newflag是1，那么没有变化。
总是返回的bts_secure_delete设更改后的定值。
*/
...
int sqlite3BtreeSetAutoVacuum(Btree *p, int autoVacuum){
  ...
  if( (pBt->btsFlags & BTS_PAGESIZE_FIXED)!=0 && (av ?1:0)!=pBt->autoVacuum ){
    ...
    pBt->autoVacuum = av ?1:0; /*av如果为非0，auto-vacuum mode启动*/
...
*/
/*
**得到一个参考的数据库文件pPage1。这也将对该文件获得读锁。
**SQLITE_OK 被返回成功。如果这个文件不是一个很好的数据库文件，然后SQLITE_CORRUPT被返回。
**如果数据库被锁定，则SQLITE_BUSY 被返回。如果我们使用完它的内存，则SQLITE_NOMEM被返回。
*/
static int lockBtree(BtShared *pBt){
  int rc;              /* Result code from subfunctions */ /*在功能上的结果代码*/
  MemPage *pPage1;     /* Page 1 of the database file */ /*数据库文件的第一页*/
  int nPage;           /* Number of pages in the database */ /*数据库中的页数*/
  int nPageFile = 0;   /* Number of pages in the database file */ /*数据库文件中的页数*/
  int nPageHeader;     /* Number of pages in the database according to hdr */ /*数据库中的页数的头部*/
  ...
  */
  ...
  if( nPage>0 ){
    ...
    */
	/*
	如果写的版本是2，这应该是在沃尔玛模式访问数据库。如果日志尚未打开，打开它。
    ...
    */
	...
	/*
	对于叶数据和非叶数据，最大嵌入式部分必须是25%和最小嵌入部分必须为12.5%。
	原设计允许这些数量有所不同，但作为版本3.6.0，我们要求他们是固定的。
	*/
    ...
    if( (u32)pageSize!=pBt->pageSize ){
      ...
      */
		/*
		读数据库假设一个BtShared.pageSize页面大小的第一页，我们发现，实际上是为页面大小。
		打开数据库，将pBt->pPage1为零和返回SQLITE_OK。调用方将再次调用这个函数，用正确的页面大小。
		*/
      releasePage(pPage1);
  ...
  */
  /*
  maxlocal是有效载荷的最大存储量局部单元格。确保它是足够小，
...
*/
/*
**如果没有突出的游标和我们不在一个事务中但有读锁的数据库，
然后这个程序unrefs数据库的文件，释放读锁影响的第一页。
**如果有一个进展中的交易，这个程序是一个空操作。
*/
...
/*
如果pBt指向空文件，然后将空文件到一个新的空数据库初始化数据库的第一页。
*/
...
/*
**尝试启动新的事务。如果另一个参数为非零，则为非零，则为读事务。
如果二个参数是2个或多个，并且独占事务被启动，这意味着没有其他进程可以访问数据库。
一个已经存在的交易可能无法通过调用这个例程的第二时间排他性标志只能用于一个新的交易升级为独家。
...
一个或另一个进程必须让路或有没有进展。返回SQLITE_BUSY不调用回调时忙已经有一个读锁，我们鼓励放弃让B进行。
...
*/
  ...
  /* Write transactions are not possible on a read-only database */  /*在只读数据库中写入事务是不可能的*/
  ...
  */
  /*
  如果一个数据库句柄已开通写事务在这共享的B树结构和请求第二写事务，则返回SQLITE_LOCKED。
  */
  ...
  ** on page 1, the transaction cannot be opened. */
  /*
  任何只读或读写事务意味着读锁页为1。因此，如果一些其他共享缓存客户机已经有一个写锁页为1，则该事务不能被打开。
  */
  ...
  do {
    ...
    */
	  /*
	  调用lockBtree()函数直到pBt->pPage1被填充或者lockBtree()函数返回SQLITE_OK以外的东西。
	  lockbtree()函数可能返回SQLITE_OK但是pBt->pPage1设置为0如果读的页为1发现页面大小的
	  数据库文件不是pBt->pageSize。在这种情况下lockbtree()将更新pBt->pageSize的页面文件大小的磁盘上。
	  */
  ...
  if( rc==SQLITE_OK ){
    ...
    if( wrflag ){
      ...
      */
	  /*
	  如果数据库大小的头部是错误的(因为这可能是一个老客户写数据库文件),则现在更新。
	  这样做迟早意味着数据库大小可以安全地重新读取数据库大小页为1，如果一个保存点回滚或事务发生在事务。
	  */
...
trans_begun:
  if( rc==SQLITE_OK && wrflag ){
    ...
    */
	  /*
	  这个调用让具有开放的事务数量正确进行确认。如果二次参数大于0和sub-journal是不是已经打开，那么它将在这里打开。
	  */
...
*/
/*
所有孩子页pPage设置指针映射项。同时，如果页包含的单元格指向溢出页，设置溢出页的指针映射项。
*/
static int setChildPtrmaps(MemPage *pPage){
  int i;                             /* Counter variable */  /*计数器的变量*/
  int nCell;                         /* Number of cells in page pPage */  /*页面中的单元格数*/
...
*/
/*
**从pPage指向iFrom页。修改该指针使其指向ITO。参数eType描述要修改指针的类型，如下：
...
static int modifyPagePointer(MemPage *pPage, Pgno iFrom, Pgno iTo, u8 eType){
  ...
  if( eType==PTRMAP_OVERFLOW2 ){
    /* The pointer is always the first 4 bytes of the page in this case.  *//*在这种情况下,指针总是第一个4字节的页。*/
...
*/
/*
**将打开的数据库页pDbPage数据库中的位置iFreePage。pDbPage仍然有效的参考。
**isCommit标志表示不需要记住日志而需要sync() ED在数据库页面pDbPage->pgno 
可以写。调用者已经答应不写该页面。
*/
static int relocatePage(
  BtShared *pBt,           /* Btree */
  MemPage *pDbPage,        /* Open page to move */  /*打开页面移动*/
  u8 eType,                /* Pointer map 'type' entry for pDbPage */  /*指针'type'是入口为pDbPage*/
  ...
  */
  ...
  /*
  **如果pDbPage是B树页，那么它可能有子页面和/或单元格指向溢出页。所有这些页的指针映射项需要更改。
  **如果pDbPage是溢出页，然后第一个4字节可以存储一个指向随后溢出页。如果是这样的情况，则该指针映射需要为随后的溢出页进行更新。
  */
  ...
  */
  /*
  修复数据库指针iPtrPage页指向iDbPage使它指向iFreePage。还将iPtrPage指针映射项。
  */
...
/* Forward declaration required by incrVacuumStep(). */ /*声明要求incrVacuumStep()。*/
...
*/
/*
**单步执行一个渐进的incremental-vacuum。如果成功，返回SQLITE_OK。
如果没有工作要做（因此再次调用这个函数没有点），返回SQLITE_DONE。
**更具体地说，这个函数试图重新组织数据库，最后一页的文件正在使用不再使用。
**如果nFin参数不为零，这个函数假定调用者会一直调用incrVacuumStep()直到返回SQLITE_DONE或错误，
并且nFin是网页数据库文件将包含在这个过程中的数量是完整的。如果nFin是零，它是假定incrVacuumStep()
将被称为一个有限的时间，可能会或可能不会空自由列表。
*/
static int incrVacuumStep(BtShared *pBt, Pgno nFin, Pgno iLastPg){
  Pgno nFreeList;           /* Number of pages still on the free-list */ /*仍在自由列表上的页数*/
  int rc;
  ...
  if( !PTRMAP_ISPAGE(pBt, iLastPg) && iLastPg!=PENDING_BYTE_PAGE(pBt) ){
    ...
    if( eType==PTRMAP_FREEPAGE ){
      if( nFin==0 ){
        ...
        */
		  /*
		  从“文件”列表中删除该页。如果nFin是非零。在这种情况下，此函数返回后，自由列表将被截断为零，
		  因此，它没有关系，如果它仍然包含一些垃圾分类。
		  */
        Pgno iFreePg;
        MemPage *pFreePg;
      ...
      Pgno iFreePg;             /* Index of free page to move pLastPg to */  /*自由移动到页面索引pLastPg*/
      MemPage *pLastPg;
      ...
      */
      /* 如果nFin是零,这个循环完全运行一次和页面pLastPg与第一次自由交换了空闲列表页。
      ...
      */
      do {
        ...
        releasePage(pFreePg);
      }while( nFin!=0 && iFreePg>nFin );   /* 判断nFin是否等于0，重复循环释放页面 */
      assert( iFreePg<iLastPg );
...
int sqlite3BtreeIncrVacuum(Btree *p){
  ...
  if( !pBt->autoVacuum ){
    rc = SQLITE_DONE;			 /* return SQLITE_DONE */
  }else{			
...
static int autoVacuumCommit(BtShared *pBt){
  ...
  if( !pBt->incrVacuum ){
    ...
    nFin = nOrig - nFree - nPtrmap;				 /* nFin equal to nOrig minus nFree and nPtrmap*/
    if( nOrig>PENDING_BYTE_PAGE(pBt) && nFin<PENDING_BYTE_PAGE(pBt) ){
      nFin--;									 /* if nFin and nOrig aren't PENDING_BYTE_PAGE(pBt),nfin minus one*/
    }
    ...
    for(iFree=nOrig; iFree>nFin && rc==SQLITE_OK; iFree--){
      rc = incrVacuumStep(pBt, nFin, iFree);	 /*  Circulation of iFree generation into the incrVacuumStep(pBt, nFin, iFree)*/
    }
...
int sqlite3BtreeCommitPhaseOne(Btree *p, const char *zMaster){
  ...
  if( p->inTrans==TRANS_WRITE ){     /*若没有写事务，此调用为空操作*/
    ...
    if( pBt->autoVacuum ){
      ...
      if( rc!=SQLITE_OK ){			/* If is not equal to SQLITE_OK, the release of the sqlite3Btree*/
        sqlite3BtreeLeave(p);
        return rc;
      }
    }
...
int sqlite3BtreeCommit(Btree *p){
  ...
  if( rc==SQLITE_OK ){
    rc = sqlite3BtreeCommitPhaseTwo(p, 0);/* 两阶段提交*/
  }
  sqlite3BtreeLeave(p);
  return rc;
}
...
void sqlite3BtreeTripAllCursors(Btree *pBtree, int errCode){
  ...
  for(p=pBtree->pBt->pCursor; p; p=p->pNext){
    ...
    p->eState = CURSOR_FAULT;		/* 例程被调用时发生反转*/
    p->skipNext = errCode;
    for(i=0; i<=p->iPage; i++){
...
*/
/*回滚事务，使所有游标失效*/
/* 这将释放写入锁在数据库文件中。如果没有活跃的游标时,也会释放读锁。*/
  ...
  if( p->inTrans==TRANS_WRITE ){/*释放数据库中的写锁*/
    ...
    ** sure pPage1->aData is set correctly. */
    if( btreeGetPage(pBt, 1, &pPage1, 0)==SQLITE_OK ){
      ...
      pBt->nPage = nPage;
      releasePage(pPage1);		/* 确保pPage1->aData 设置正确*/
    }
...
int sqlite3BtreeSavepoint(Btree *p, int op, int iSavepoint){
  ...
  if( p && p->inTrans==TRANS_WRITE ){
    BtShared *pBt = p->pBt;
    assert( op==SAVEPOINT_RELEASE || op==SAVEPOINT_ROLLBACK );  /*声明 op为SAVEPOINT_ROLLBACK或SAVEPOINT_RELEASE*/
    ...
    if( rc==SQLITE_OK ){
      ...
      rc = newDatabase(pBt);
      pBt->nPage = get4byte(28 + pBt->pPage1->aData);   /* 数据库的大小是写进标题的抵消28事务开始*/
 ...
 */
  ...
  ** variables and link the cursor into the BtShared list.  */
  ...
  pBt->pCursor = pCur;				/* 填写BtCursor游标变量*/
  pCur->eState = CURSOR_INVALID;
  pCur->cachedRowid = 0;
  return SQLITE_OK;
}
...
*/
	/*
	**返回一个BtCursor对象的大小(以字节计)。	
	**需要该接口使游标可以预先分配
	**足够的存储空间。该BtCursor对象对用户是不透明的
	**对用户，以便他们不能用sizeof（）- 他们必须调用
	**此程序。
	*/
...
*/
/*
**初始化存储器将被转换成一个BtCursor对象。
**
**这里一个简单方法是用memset将整个对象置为
**零。但事实证明，apPage[]和aiIdx[]数组
**不需要进行调零，他们比较大，所以我们通过跳过这些元素的初始化，
可以节省很多的运行时间。
*/
...
int sqlite3BtreeCloseCursor(BtCursor *pCur){
  ...
  if( pBtree ){
    ...
    unlockBtreeIfUnused(pBt);			/* 数据库文件的读锁被释放*/
    invalidateOverflowCache(pCur);
    /* sqlite3_free(pCur); */
    sqlite3BtreeLeave(pBtree);
  }
...
*/
#ifndef NDEBUG
/*保证BtCursor有有效的BtCursor.info结构。若无效，调用btreeParseCell()去填充*/
  static void assertCellInfo(BtCursor *pCur){
    CellInfo info;
    int iPage = pCur->iPage;
    memset(&info, 0, sizeof(info));
    btreeParseCell(pCur->apPage[iPage], pCur->aiIdx[iPage], &info);
    assert( memcmp(&info, &pCur->info, sizeof(info))==0 );	/* 缓存减少了调用的数量btreeParseCell()*/
  }
#else
/* 这里有一个错误在 getCellInfo()实现为一个宏,这些版本时导致编译器崩溃的MSVC*/
...
*/
	/*
	**如果给定的BtCursor是有效的,返回true。一个有效的游标
	**当前指向中的行是非空的表。
	**这是一个验证程序只在assert()语句中使用。
	*/
...
*/
/*pSize为buffer的大小，buffer用来保存当前入口(用pCur指向)的key值。*/
int sqlite3BtreeKeySize(BtCursor *pCur, i64 *pSize){
  assert( cursorHoldsMutex(pCur) );
  assert( pCur->eState==CURSOR_INVALID || pCur->eState==CURSOR_VALID );
  if( pCur->eState!=CURSOR_VALID ){/*游标指向无效入口，pSize = 0*/
    *pSize = 0;
  }else{
    getCellInfo(pCur);
    *pSize = pCur->info.nKey;
  }
  return SQLITE_OK;				/* 为表设置了INTKEY标志,这个例程返回键本身,而不是在关键的字节数*/
}
...
int sqlite3BtreeDataSize(BtCursor *pCur, u32 *pSize){
  ...
  *pSize = pCur->info.nData;/*用pSize返回数据长度*/
  return SQLITE_OK;		/* 失败是不可能的。这个函数始终返回SQLITE_OK。*/
}
...
static int getOverflowPage(
  ...
  assert( sqlite3_mutex_held(pBt->mutex) );
  assert(pPgnoNext);		/* 页码链接列表中的下一个溢出页写入* pPgnoNext*/
  ...
  if( pBt->autoVacuum ){
    ...
    if( iGuess<=btreePagecount(pBt) ){
      ...
      if( rc==SQLITE_OK && eType==PTRMAP_OVERFLOW2 && pgno==ovfl ){
        next = iGuess;			/* 如果想证明是错误的,回到加载数据的页码ovfl来确定下一个页码*/
        rc = SQLITE_DONE;
      }
    }
  }
    ...
    if( pCur->isIncrblobHandle && !pCur->aOverflow ){
      ...
      ** been used instead of this routine. */
      if( ALWAYS(nOvfl) && !pCur->aOverflow ){    
        rc = SQLITE_NOMEM;
	  /* nOvfl总是正的。如果是零,fetchPayload用来代替这个例程*/
      }
    }
    ...
    for( ; rc==SQLITE_OK && amt>0 && nextPage; iIdx++){
	 ...
	 /* 如果需要,填充缓存溢出文章列表*/
      ...
      if( offset>=ovflSize ){
        ...
        ** function.
        */
        /*阅读此页的唯一原因是为了获得该页面在溢出页链表中的下一个页号。
...
#endif
		/* 
		** 然后可以直接从数据库文件中读取数据到输出缓冲区,
		** 完全绕过页面缓存。这加速加载大跨许多溢出页的记录。
		*/
        {
          DbPage *pDbPage;
...
int sqlite3BtreeKey(BtCursor *pCur, u32 offset, u32 amt, void *pBuf){
  assert( cursorHoldsMutex(pCur) );		/* 必须确保pCur指向一个有效的表中的行。*/
...
int sqlite3BtreeData(BtCursor *pCur, u32 offset, u32 amt, void *pBuf){
  ...
  assert( cursorHoldsMutex(pCur) );
  rc = restoreCursorPosition(pCur);
  if( rc==SQLITE_OK ){				 /* 在成功或错误代码返回SQLITE_OK如果出了什么差错。*/
    assert( pCur->eState==CURSOR_VALID );
    assert( pCur->iPage>=0 && pCur->apPage[pCur->iPage] );
    assert( pCur->aiIdx[pCur->iPage]<pCur->apPage[pCur->iPage]->nCell );
    rc = accessPayload(pCur, offset, amt, pBuf, 0);   /* 返回一个错误如果“抵消+ amt”大于可用的负载。*/
  }
  return rc;
}
...
static const unsigned char *fetchPayload(
  ...
  *pAmt = nLocal;
  return aPayload; /* 重组键数据并将其复制到一个预先分配的缓冲区*/
}
...
*/
	...
	**返回的指针是短暂的。这个程序在没有溢出页的使用常见的情况，用于快速访问key 和 data和。
	*/
...
** the page.
*/
/* 如果iIdx等于pParent分支的总数,这个页码iChild就是的右孩子页面。*/
...
*/
	/*
	将游标移动到父页面。 pCur-> IDX被设定为包含指向页的指针的单元索引。如果为最右边的子页面，pCur-> IDX被设置为比最大的单元格数大1。
	*/
...
/* 移动光标指向的根页b -树结构。*/
/* 
** 如果表有一个虚拟根目录页面,光标点转移到虚拟根页面而不是实际的根。
** 表有一个虚拟的根页当实际根页面不包含细胞和单个子页面。这只能发生在第1页的表。
*/
static int moveToRoot(BtCursor *pCur){
  ...
  if( pCur->iPage>=0 ){		/* 如果b -树结构是空的*/
    ...
    ** return an SQLITE_CORRUPT error.  */
    ...
    if( (pCur->pKeyInfo==0)!=pCur->apPage[0]->intKey ){
      return SQLITE_CORRUPT_BKPT;		/* 返回一个SQLITE_CORRUPT错误*/
    }
  }
  ...
  */
  ...
  if( pRoot->nCell==0 && !pRoot->leaf ){
    Pgno subpage;
    if( pRoot->pgno!=1 ) return SQLITE_CORRUPT_BKPT;/* 如果的假设是不正确的,它不可能标志字节被修改*/
    subpage = get4byte(&pRoot->aData[pRoot->hdrOffset+8]);
    pCur->eState = CURSOR_VALID;		/* 这个游标是页面的引用*/
...
** 设置*pRes为0,如果光标只向某一些或如果表是空的就设置*pRes1。
*/
  ...
  ** to move to, then just return without doing any work */
  if( pCur->eState==CURSOR_VALID && pCur->validNKey 
   && pCur->apPage[0]->intKey 
    ...
    if( pCur->info.nKey==intKey ){		/* 如果光标已经定位在我们试图移动,然后返回 */
      *pRes = 0;
      return SQLITE_OK;
    }
  ...
  for(;;){
    ...
    ** a moveToChild() or moveToRoot() call would have detected corruption.  */
    assert( pPage->nCell>0 );	/* pPage - > nCell必须大于零。*/
    assert( pPage->intKey==(pIdxKey==0) );
    lwr = 0;
    upr = pPage->nCell-1;
    if( biasRight ){	/*如果这是上面的根页光标是无效的,这for循环不会运行。*/
		...
		*/
        ...
        if( nCell<=pPage->max1bytePayload
          ...
          ** fits entirely on the main b-tree page.  */
          testcase( pCell+nCell+2==pPage->aDataEnd ); /* 如果是两个字节*/
		 ...
		 /* 记录流到一个或多个溢出页。在这种情况下,需要解析完整的分支*/
          ...
          if( pCellKey==0 ){
            rc = SQLITE_NOMEM;
            goto moveto_finish;		/* 如果pCellKey为0，直接跳转完成*/
          }
...
int sqlite3BtreeEof(BtCursor *pCur){
  ...
  ** 如果游标在CURSOR_REQUIRESEEK但所有表项删除什么?这个API将需要更改返回一个错误代码以及布尔值。
  */
