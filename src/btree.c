...
static int querySharedCacheTableLock(Btree *p, Pgno iTab, u8 eLock){
  ...
  **【王】如果需要一个写锁，那么B树必须有一个开放的写事务。显然，为了达到这种效果
  ** 文件本身必须有一个开放的写事务。
  */
...
** into *pRC.
** 写一个条目进入指针位图。这个程序更新页码“key”的指针位图入口，
...
** 从指针位图读取入口。
** 这个程序检索页面 'key'的指针位图入口，分别将类型和父页码写入到*pEType 和 *pPgno中。
** 如果运行出错则返回错误代码，其他返回SQLITE_OK.
*/
/*读取pointer map的入口，写入pEType和pPgno中*/
...
*/
/*如果pCell(pPage的一部分)包含指向溢出页的指针，则为这个溢出页插入一个入口到pointer-map*/
...
static int defragmentPage(MemPage *pPage){
  ...
  int hdr;                   /* Offset to the page header */         //页头部得偏移量
  int size;                  /* Size of a cell */                    //单元的大小
  int usableSize;            /* Number of usable bytes on a page */  //页上可用单元的数量
  int cellOffset;            /* Offset to the cell pointer array */  //单元指针数组的偏移量
  int cbrk;                  /* Offset to the cell content area */   //单元内容区域的偏移量
  int nCell;                 /* Number of cells on the page */       //页上单元的数量
  unsigned char *data;       /* The page data */                     //页数据
  unsigned char *temp;       /* Temp area for cell content */        //单元内容的临时区
  int iCellFirst;            /* First allowable cell index */        //第一个许可的单元索引
  int iCellLast;             /* Last possible cell index */          //最后一个可能的单元索引
...
#endif
    assert( pc>=iCellFirst && pc<=iCellLast );/*第一个允许cell索引，最后一个可能的cell索引之间。*/
...
** no entries.  //建立一个原始页面,以便它看起来像一个数据库没有入口。
*/
...
** 对页pPage的所有孩子节点设置指针映射入口。如果pPage包含指向溢出页的指针的单元，也对溢出页设置指针映射入口。
*/
...
** 移动开放数据库页pDbPage到数据库中的安放位置iFreePage。pDbPage参数仍可用。
...
static int relocatePage(
  ...
  u8 eType,                /* Pointer map 'type' entry for pDbPage */    //pDbPage指针映射类型入口
  Pgno iPtrPage,           /* Pointer map 'page-no' entry for pDbPage */ //pDbPage指针映射'page-no'入口
  ...
  ** 如果pDbPage是btree-page,那么它可能有孩子页或指向溢出页的单元。所有这些指针映射入口都需要更改。
  ...
  ** iPtrPage.
  ** 固定数据库指针到页iPtrPage上，该页指向iDbPage,以便它指向iFreePage。同时对于iPtrPage，固定指针映射入口。
  */
...
static int autoVacuumCommit(BtShared *pBt){
  ...
  if( !pBt->incrVacuum ){
    ...
    int nEntry;        /* Number of entries on one ptrmap page */             //在一个ptrmap页上的入口数
...
*/
void sqlite3BtreeCursorZero(BtCursor *p){  //用memset()将整个对象置为零
...
** pSize为buffer的大小，buffer用来保存当前入口(用pCur指向)的key值。如果游标未指向一个有效的入口,* pSize设置为0。
...
** 设置*pSize的数据域的字节数，*pSize 在当前游标指向的入口中。
** The caller must guarantee that the cursor is pointing to a non-NULL
** valid entry.  In other words, the calling procedure must guarantee
** that the cursor has Cursor.eState==CURSOR_VALID.
** 调用者必须保证游标指向一个非空有效入口。换句话说,调用程序必须保证游标Cursor.eState = = CURSOR_VALID。
...
*/
	/*
	**对于游标pCur正指向的入口，此功能用于读或覆写有效载荷信息。
	...
	** 且当前游标入口使用一个或多个溢出页,这个函数分配空间和慢慢增加溢出页列表缓存数组(BtCursor.aOverflow)。
...
static int accessPayload(          //读或覆写有效载荷信息
  BtCursor *pCur,      /* Cursor pointing to entry to read from */     //游标指向要读取数据的入口
  ...
  u32 nKey;
  int iIdx = 0;
  MemPage *pPage = pCur->apPage[pCur->iPage]; /* Btree page of current entry */  //当前入口的B树页
...
** a valid pointer.
** 返回从pCur游标正在指向的入口到有效载荷的指针。如果skipKey==0 则该指针指向关键字的开
...
*/
	...
	*/
static const unsigned char *fetchPayload(       //返回从pCur游标正在指向的入口到有效载荷的指针
  BtCursor *pCur,      /* Cursor pointing to entry to read from */      //指向要读取入口的游标
...
const void *sqlite3BtreeKeyFetch(BtCursor *pCur, int *pAmt){  //返回游标pCur指向条目的key 在本地B树页上可用的字节数
  const void *p = 0;
...
const void *sqlite3BtreeDataFetch(BtCursor *pCur, int *pAmt){  //返回游标pCur指向条目的data在本地B树页上可用的字节数
  const void *p = 0;
  assert( sqlite3_mutex_held(pCur->pBtree->db->mutex) );
  assert( cursorHoldsMutex(pCur) );
  if( ALWAYS(pCur->eState==CURSOR_VALID) ){
...
**  是找到最左边*entry*下的入口，而moveToRightmost()是找到最右边*page*下的入口。
...
** 移动游标以便于指向被pIdxKey 或intKey指向的入口附近的关键字。
...
*/
	/*
	**此功能用于复制的b树节点的存储内容。如果页面pFrom中不是叶子页，然后
	每个子页指针的映射条目进行更新。	 
	 返回之前，页面PTO使用btreeInitPage重新初始化（）。此功能的性能不是关键的。
	 它仅由balance_shallower（）和balance_deeper（）程序组成。
	*/
...
static int btreeCreateTable(Btree *p, int *piTable, int createTabFlags){
  ...
  if( pBt->autoVacuum ){
    ...
    */
    invalidateAllOverflowCache(pBt);/*页为溢出页，删除溢出页映射*/
...
/* 在数据库文件中新建一个空的B-tree，是采用表格式（B+tree）,还是索引格式(B-tree)*/
...
*/
/*
	把meta-information写入数据库。Meta[0]为只读，可能不会被写。
*/
...
int sqlite3BtreeCount(BtCursor *pCur, i64 *pnEntry){
  ...
  */
  /*除非错误发生，以下循环在每一个B-Tree结构中执行，除了溢出页*/
  while( rc==SQLITE_OK ){
    ...
    ** caller.
    */
    /*pPage是一个叶子页。这个循环使这个光标指向第一个内部的单元格。这个单元格指向树中没有被访问的下一页的
...
*/
/*页iPage的参考的数量加1。如果为页中的第二个参考，加错误消息到pCheck->zErrMsg里。
如果页中有2个或者更多的参照，返回1。如果这个页被第一次引用，返回0。
*/
...
aRoot中的入口数量为nRoot。在这个函数被调用之前，一个只读或者读写事务必须为打开状态。
写错误数量到*pnErr。
*/
