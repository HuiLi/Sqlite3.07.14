...
** 这个文件实现外部(基于磁盘)数据库使用B树结构.在“btreeInt.h”文件中查看声明的附加信息.
** 包括文件格式描述和操作概述.
*/
/*
【潘光珍】
...
*/
int sqlite3_enable_shared_cache(int enable) {//启用或禁用共享的页和模式
...
*/
static int hasSharedCacheTableLock( //检查pBtree是否持有所需的锁来读取或写入表的根页iRoot
	...
	** read-lock on iTab will suffice. Return 1 if any of these are found.  */
	/*搜索所需的锁(pLock).在根页iTAB上的写锁(pLock->eLock==WRITE_LOCK) ,在架构表上的写锁( pLock->iTable==1),
	或(如果客户正在读)iTab上的读锁就足够了(pLock->eLock>=eLockType,eLockType为所需要的锁).如果上述情况出现就返回1.
	*/
	/*
...
*/
static int hasReadConflicts(Btree *pBtree, Pgno iRoot) {//是否有读冲突
	BtCursor *p;
...
*/
/*
查询,判断在iTab根页的表上B树句柄p是否能获取eLock类型的锁(READ_LOCK或WRITE_LOCK)(eLock==READ_LOCK || eLock==WRITE_LOCK ).
...
如果通过调用 setSharedCacheTableLock（），可以获得锁，返回SQLITE_OK。否则返回SQLITE_LOCKED。
*/
static int querySharedCacheTableLock(Btree *p, Pgno iTab, u8 eLock) {//在iTab根页的表上B树句柄p是否能获取eLock类型的锁
	BtShared *pBt = p->pBt;
...
** 在带有根页iTable的表上添加锁到被B树句柄p使用的共享B树上.参数eLock必须是READ_LOCK或 WRITE_LOCK.
** 此功能假定如下：
...
#define invalidateIncrblobCursors(x,y,z)
																...
																** 设定位向量BtShared.pHasContent的位号.这个函数将在先前包含数据的页
																** 变为一个空闲列表叶节点页的时候被调用.
																**
																** BtShared.pHasContent位向量存在是为了解决未知的错误,这个错误是发生在空闲列表叶节点页的
																** 临近节点两个有用的IO优化的相互作用产生的：
																** 1)当页的所有数据都被删除并且页变成空闲列表表叶节点页的时候,这个页将不能被写到数据库中
																**   (空闲列表表叶节点的页中包含的是无效的数据).有时这样的页甚至不会被记录到日志中.
																**	 (因为它并没有被修改所有没有必要记录到日志)
																** 2)当空闲列表表叶节点的页被重新使用的时候,它的不会从数据库中或已经被写的日志文件中读取.
																**    因为它是不是都有意义的.
																**
																...
																**
																** 该解决方案就是BtShared.pHasContent 位向量.每当一个页面移除成为空闲列表表叶子节点页时,
																** 相应的位将在位向量中设置.每当一个叶节点页从空闲列表表中提取,如果相应的位已经在
...
** 保存当前游标的位置用变量BtCursor.nKey和BtCursor.pKey存储.游标的状态被设置为CURSOR_REQUIRESEEK.
...
*/
static int saveAllCursors(BtShared *pBt, Pgno iRoot, BtCursor *pExcept) {//保存所有在有根页iRoot的表上开放的游标的位置
	BtCursor *p;
	assert(sqlite3_mutex_held(pBt->mutex));
...
/* Clear the current cursor position.  */   /*【潘光珍】删除当前游标所在位置*/
void sqlite3BtreeClearCursor(BtCursor *pCur) {// 清除当前游标位置
	assert(cursorHoldsMutex(pCur));
...
static int btreeMoveto(
	BtCursor *pCur,     /* Cursor open on the btree to be searched */              //在将被搜索的B树上开放游标
	...
	int *pRes           /* Write search results here */                            /*【潘光珍】搜索的结果写在这里*/
	) {
...
static int btreeRestoreCursorPosition(BtCursor *pCur) {
	...
	if (pCur->eState == CURSOR_FAULT) {
		return pCur->skipNext;  /* Prev() is noop if (skipNext) negative. Next() is noop if positive */
								//如果skipNext是负的则 Prev()无操作.如果为正则Next()无操作.
	}
	pCur->eState = CURSOR_INVALID;
...
static void ptrmapPut(BtShared *pBt, Pgno key, u8 eType, Pgno parent, int *pRC) {
	...
	int rc;           /* Return code from subfunctions(子函数) */     //从子函数返回代码
...
*/
static int ptrmapGet(BtShared *pBt, Pgno key, u8 *pEType, Pgno *pPgno) {//从指针位图读取条目
	DbPage *pDbPage;   /* The pointer map page */             //指针位图页        /*【潘光珍】Pager的页句柄*/
...
** 解码一个页的标记字节(头部的第一个字节)并初始化的MemPage相应结构域。
** 只支持以下组合.任何不同都是指示一个损坏的数据文件：
**         PTF_ZERODATA
...
*/
/*【潘光珍】初始化磁盘块的辅助信息。
返回sqlite_ok成功。如果我们看到该页没有良好的数据库页面，然后返回sqlite_corrupt。
...
static int btreeInitPage(MemPage *pPage) {     //B树初始化页
	...
	if (!pPage->isInit) {
		...
		int usableSize;    /* Amount of usable space on each page */      //每个页上的可用空间的大小   /*【潘光珍】每页可用的字节数。pageSize-每页尾部保留空间的大小，在文件头偏移为20处设定。*/

		u16 cellOffset;    /* Offset from start of page to first cell pointer */   //从页面的开始到第一个单元指针的偏移量  /*【潘光珍】单元指针数组的偏移量，aData中第1个单元的指针 */
		int nFree;         /* Number of unused bytes on the page */                //页上未使用字节的大小   /*【潘光珍】可使用空间的总和（字节数） */
		...
		*/
		...
		while (pc>0) {
			...
			if ((next>0 && next <= pc + size + 3) || pc + size>usableSize) {
				...
				** 空闲块必须按升序排列。并且空闲开的最后一个字节必须在一个数据库页上*/
				return SQLITE_CORRUPT_BKPT;
			}
			nFree = nFree + size;
			pc = next;
...
** no entries.  //建立一个原始页面,以便它看起来像一个数据库页但没有条目.
*/
static void zeroPage(MemPage *pPage, int flags) {  //建立一个空的数据库页
	unsigned char *data = pPage->aData;
...
*/
int sqlite3BtreeSyncDisabled(Btree *p) {//如果在磁盘上没有sync()同步函数,则不能同步
	BtShared *pBt = p->pBt;
	int rc;
	assert(sqlite3_mutex_held(p->db->mutex));
	sqlite3BtreeEnter(p);
