...
static void *memsys3MallocUnsafe(int nByte){
	...
	if( nByte<=12 ){                //给nBlock赋值
		...
		nBlock = (nByte + 11)/8;//nByte的值是有什么决定的？？？？
	}
	assert( nBlock>=2 );//nBlock要大于等于2，我觉的是为2,3
	...
	*/   //首先在小chunk或者大chunk中寻找正确大小块的入口，一般都会成功
	if( nBlock <= MX_SMALL ){        //nBlock小于MX_SMALL，则在小chunk中找MX_SMALL=10   
		i = mem3.aiSmall[nBlock-2];//空闲chunk中第二个block的是u.list形式,这个list的头指针储存在mem3.aiSmall[].nBlock=2,3.那么nBlock-2=1或0
		if( i>0 ){
			memsys3UnlinkFromList(i, &mem3.aiSmall[nBlock-2]);//把这个块从链表中解除
			return memsys3Checkout(i, nBlock);  //返回找到的满足的chunk
		}
	}else{    //若nBlock大于MX_SMALL，则在大chunk中找
		int hash = nBlock % N_HASH;
		for(i=mem3.aiHash[hash]; i>0; i=mem3.aPool[i].u.list.next){//对hash表进行遍历，寻找合适大小，删除并返回
	...
	*/     //尝试从master chunk中分裂出合适的空间，第一步失败才执行
	if( mem3.szMaster>=nBlock ){
		return memsys3FromMaster(nBlock);  //该函数作用是从主要的空闲块中取出nBlock大小的块
	}
	...
	*///遍历整个内存池，合并相邻空闲chunk，重新计算主要的chunk大小，再次尝试从master chunk中分裂出满足分配条件的chunk。前面都不行才执行该步骤。
	for(toFree=nBlock*16; toFree<(mem3.nPool*16); toFree *= 2){  //遍历内存池//??????.    nPool 内存变量数组分配的空间大小
		memsys3OutOfMemory(toFree);     //不够分配则释放
		if( mem3.iMaster ){               //master chunk存在，将其链接到相应块索引表中        
			memsys3Link(mem3.iMaster);       // iMaster是主chunk的索引,也就是占用空间的索引
			mem3.iMaster = 0;
			mem3.szMaster = 0;
		}
		for(i=0; i<N_HASH; i++){
...
static void *memsys3Realloc(void *pPrior, int nBytes){
	...
	if( nBytes<=0 ){//猜测nBytes为可用空间？？？    
		sqlite3_free(pPrior);  //释放从由sqlite3Malloc获得的内存空间
		return 0;
	}
	nOld = memsys3Size(pPrior);  //获取未完成分配的内存空间，//以字节的方式返回未完成分配的内存大小，不返回头8byte，节约开销，此函数仅针对刚刚被check out的内存
	//*返回一个未分配的大小，以字节为单位。
	if( nBytes<=nOld && nBytes>=nOld-128 ){//为什么有具体的数字？？？？
		return pPrior;
	}
	memsys3Enter();              
	p = memsys3MallocUnsafe(nBytes);     //申请n字节的内存空间
	if( p ){
		if( nOld<nBytes ){//原内存块中已经占用的内存空间<可用的内存空间
			memcpy(p, pPrior, nOld);//吧pPrior中的内容复制到p空间中，复制的长度为nOld
		}else{
			memcpy(p, pPrior, nBytes);//
		}
		memsys3FreeUnsafe(pPrior);//释放pPrior内存空间的内容
	}
	memsys3Leave();
	return p;
}
