...
static HashElem *findElementGivenHash(
  ...
  int nKey,           /* Bytes in key (not counting zero terminator)定义一个整型的键值 */
  unsigned int h      /* The hash for this key. 定义一个无符号的整型变量h*/
)//定义一个静态的函数给哈希表中的匹配的元素定一个键值
{
  HashElem *elem;                /* Used to loop thru the element list ， 定义一个哈希表指针 */
  int count;                     /* Number of elements left to test，定义一个整形变量计算表中元素的个数 */
