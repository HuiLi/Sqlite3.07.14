...
*/
//对当前日志文件定义最大值。一个负数值意味着没有限制。这个值可能被函数sqlite3PagerJournalSizeLimit（）应用程序界面覆写。也是看“编译指示 journal-size-limit”
...
*/
//这个类型是被用于表示页面数量（页号）的。文件中的第一页被称为page1，0是被用来表示“没有页面”。
typedef u32 Pgno;
定义Pgno为u32类型的
/*
** Each open file is managed by a separate instance of the "Pager" structure.
*/
//每个打开的文件都是由一个单独的“Pager”结构实例。
...
** for details.
*/
//页码PAGER_MJ_PGNO从未在一个SQLite数据库中使用过(它是专为解决在windows / posix不兼容问题保留的)。这是在日志中运用来表示日志文件的其余部分致力于存储主日志名称-没有更多的页面用于回滚了。在pager.c看到writeMasterJournal()的函数的评论的细节。
...
#定义PAGER_JOURNALMODE_QUERY(-1)/ *查询journalmode * /的值　　
