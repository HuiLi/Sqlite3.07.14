...
** 当一个SQL语句将要开始被解析的时候，下面的这个程序将要被调用，当我们在需要的时候有必要去初始化语法解析器结构
*/
...
*/
struct TableLock {
	int iDb;             /* The database containing the table to be locked */   //iDb数据库包含被锁定的表
	int iTab;            /* The root page of the table to be locked */          //被锁定的表的根页面，存放着被所锁定的表的一切信息
	u8 isWriteLock;      /* True for write lock.  False for a read lock */      //读写锁，如果为真则是写锁，否则就是读锁
	const char *zName;   /* Name of the table */                                //表的名字
};

/*
** Record the fact that we want to lock a table at run-time.
** 记录我们在运行过程中想要去锁定的表
** The table to be locked has root page iTab and is found in database iDb. 参数iTab是被锁定的表的根页面，这个表被存放在iDb数据库中
** A read or a write lock can be taken depending on isWritelock. 置读锁或者写锁需要根据isWritelock参数来判定
**
** This routine just records the fact that the lock is desired.  The
** code to make the lock occur is generated by a later call to
** codeTableLocks() which occurs during sqlite3FinishCoding().
** 这个程序仅仅记录需要的锁（读锁还是写锁），具体生成代码的过程要在之后调用codeTableLocks()，最后调用sqlite3FinishCoding()完成编码
*/
void sqlite3TableLock(
	Parse *pParse,     /*Parsing context 语法分析上下文*/
	int iDb,           /* Index of the database containing the table to lock  存放被锁定表的数据库*/
	int iTab,          /* Root page number of the table to be locked  被锁定的表的根页面 */
	u8 isWriteLock,    /* True for a write lock 如果加了一个写操作锁则将该位置位为true*/
	const char *zName  /* Name of the table to be locked  被加锁的表的名字*/
	){
	Parse *pToplevel = sqlite3ParseToplevel(pParse);  
	int i;
	int nBytes;
	TableLock *p;    //锁表指针
	assert(iDb >= 0);    //iDb大于0意味着确实有锁表已经被存放到了iDb数据库中，只有这个断言正确下面的操作才能进行

	//下面的for循环是在检索锁表，并且决定是置成读锁还是写锁
	for (i = 0; i < pToplevel->nTableLock; i++){
		p = &pToplevel->aTableLock[i];   //取锁表数组的第i个单元的地址，将地址赋值给p（p指针实际上是锁表指针TableLock *p）指针
		if (p->iDb == iDb && p->iTab == iTab){  //判断被锁定的表和数据库是否是通过参数传递过来的表和数据库，如果不是，那么我们将什么都不做直接退出函数
			p->isWriteLock = (p->isWriteLock || isWriteLock);  //判断究竟是读锁还是写锁，将这个参数置成相应的读写锁，置读锁还是写锁需要根据这个参数
			return;  
		}
	}
...
** 通过调用sqlite3TableLock()，为每一个已经加锁的表编码一条OP_TableLock指令
*/
static void codeTableLocks(Parse *pParse){
	int i;
	Vdbe *pVdbe;  //VDBE引擎

	pVdbe = sqlite3GetVdbe(pParse);   //获得VDBE的支持
	...
	for (i = 0; i<pParse->nTableLock; i++){
		...
		sqlite3VdbeAddOp4(pVdbe, OP_TableLock, p1, p->iTab, p->isWriteLock,
			p->zName, P4_STATIC);   //根据 p->isWriteLock判断究竟是读锁还是写锁，然后将其操作复制给OP_TableLock
	}
}
#else
