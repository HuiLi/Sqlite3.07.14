...
void sqlite3SelectPrep(  /**/
  Parse *pParse,         /*  解析器的上下文 */
  Select *p,             /*  被编码的select表达式 */
  NameContext *pOuterNC  /*  命名上下文 */
){
  sqlite3 *db;/* 定义数据库指针变量*/
  if( NEVER(p==0) ) return;如果/*判断传递过来的参数select表达式指针是否为空*/
  db = pParse->db;/*数据库指针变量指向解析器上下文对象的数据库存储域*/
  if( p->selFlags & SF_HasTypeInfo ) return;/*判断传递过来的参数select表达式指针与SF_HasTypeInfo进行与运算后的结果*/
  sqlite3SelectExpand(pParse, p);/*调用sqlite3SelectExpand方法，传递两个参数：解析器和表达式指针变量*/
  if( pParse->nErr || db->mallocFailed ) return;/*判断解析器的是否出错或数据库分配内存是否成功*/
  sqlite3ResolveSelectNames(pParse, p, pOuterNC);/*调用sqlite3ResolveSelectNames方法，传递三个参数：解析器，表达式指针，命名上下文指针*/
  if( pParse->nErr || db->mallocFailed ) return;/*再次判断解析器的是否出错或数据库分配内存是否成功*/
  sqlite3SelectAddTypeInfo(pParse, p);/*调用sqlite3SelectAddTypeInfo方法，传递两个参数：解析器，表达式。将查询语句添加进解析器。*/
}
...
**OP_AggFinalize操作码
*/
static void finalizeAggFunctions(Parse *pParse, AggInfo *pAggInfo){/*定义一个函数，传递两个参数*/
  Vdbe *v = pParse->pVdbe;/*定义指针变量，指向解析器的pVdbe域*/
  int i;
  struct AggInfo_func *pF;/*定义指针变量，指向AggInfo_func类型的函数*/
  for(i=0, pF=pAggInfo->aFunc; i<pAggInfo->nFunc; i++, pF++){/*循环调用sqlite3VdbeAddOp4*/
...
*/
static void updateAccumulator(Parse *pParse, AggInfo *pAggInfo){/*定义函数，更新聚合器*/
  Vdbe *v = pParse->pVdbe;//定义指针变量
  int i;
  int regHit = 0;
  int addrHitTest = 0;
  struct AggInfo_func *pF;//定义函数指针变量
  struct AggInfo_col *pC;

  pAggInfo->directMode = 1;
  sqlite3ExprCacheClear(pParse);//调用函数，清除语法分析器缓存
  for(i=0, pF=pAggInfo->aFunc; i<pAggInfo->nFunc; i++, pF++){//循环实现
    int nArg;
    int addrNext = 0;
    int regAgg;
    ExprList *pList = pF->pExpr->x.pList;
    assert( !ExprHasProperty(pF->pExpr, EP_xIsSelect) );
    if( pList ){//判断表达式列表是否为空
      nArg = pList->nExpr;//
...
*/
int sqlite3Select(
  Parse *pParse,         /* The parser context 。语法分析器上下文*/
  Select *p,             /* The SELECT statement being coded。被编码的选择语句*/
  ...
  SrcList *pTabList;     /* List of tables to select from 。from子句列表*/
  Expr *pWhere;          /* The WHERE clause.  May be NULL */
  ExprList *pOrderBy;    /* The ORDER BY clause.  May be NULL。排序子句 */
  ExprList *pGroupBy;    /* The GROUP BY clause.  May be NULL 。分组*/
  Expr *pHaving;         /* The HAVING clause.  May be NULL */
  int isDistinct;        /* True if the DISTINCT keyword is present 。如果存在不同关键字返回True*/
  int distinct;          /* Table to use for the distinct set 。用于不同集合的表*/
  int rc = 1;            /* Value to return from this function 。返回值*/
  int addrSortIndex;     /* Address of an OP_OpenEphemeral instruction 。索引指令地址*/
  int addrDistinctIndex; /* Address of an OP_OpenEphemeral instruction 。索引指令地址 */
  ...
  sqlite3SelectPrep(pParse, p, 0);//调用sqlite3SelectPrep方法，执行预处理
  pOrderBy = p->pOrderBy;//指向下一个节点
  pTabList = p->pSrc;
  pEList = p->pEList;
